...
//
import tr.org.tspb.constants.exceptions.WorkflowExcepiton;
import tr.org.tspb.workflow.process.MyStateMachine;
import tr.org.tspb.workflow.process.MyStateMachineFactory;
...
public class WorkFlowCtrl implements FmsWorkFlow {
    ...
    public List<Document> getHistory() {
        ...
        if (crudObject != null) {
            Document doc = mongoDbUtil.
                    findOne(myForm.getDb(), myForm.getTable(), Filters.eq(
                            MONGO_ID, crudObject.get(MONGO_ID)));
    ...
    @Override
    public void init(FmsForm myForm, MyMap crudbject, Document filter) throws
            FormConfigException {
        this.myForm = myForm;
    ...
    public void trigWorkFlow(MyCommandResult smControlResult) {
        ...
        if (myStateMachine != null) {
            ...
            bringGuiImpactOfStateMachine(myStateMachine.getStateMachine(),
                    myRule, smControlResult);
        }
    }
    ...
    public String createModel() {
        ...
        model.getDefaultConnectionOverlays().
                add(new ArrowOverlay(20, 20, 1, 1));
        ...
        for (MyRule myRule : myForm.getWorkflowSteps()) {
            ...
            for (MyTransition myTransition : myRule.getTransitions()) {
                ...
                EndPoint currentEndPoint = new BlankEndPoint(
                        sourceEndPointAnchor);//DotEndPoint
                el.addEndPoint(currentEndPoint);

                Element nextElement = getElement(myTransition.getState());
                EndPoint nextEndPoint = new BlankEndPoint(
                        destinationEndPointAnchor);

                if (nextElement == null) {
                    throw new RuntimeException(myTransition.getState().
                ...
                conn.getOverlays().
                        add(new LabelOverlay(myTransition.getTrigger(),
                                "ui-diagram-label", 0.5));
    ...
    public void armWorkflowRules() {
        for (MyRule myRule : myForm.getWorkflowSteps()) {
            try {
                ...
                myRule.arrangeActions(myForm, loginController.getRoleMap(),
                        filter);
            } catch (Exception ex) {
    ...
    private void refreshCanStart() {
        Document document = mongoDbUtil.findOne(myForm.getDb(), myForm.
                getTable(), Filters.eq(MONGO_ID, crudObject.get(MONGO_ID)));
    ...
    private String localShowWorkFlow() throws WorkflowExcepiton {
        ...
        if (crudObject == null || crudObject.isNew()) {
            throw new WorkflowExcepiton(
                    "could not start workflow process on null or new object");
        }

        Document crudDoc = mongoDbUtil.
                findOne(myForm.getDb(), myForm.getTable(), Filters.eq(MONGO_ID,
                        crudObject.get(MONGO_ID)));
        ...
        if (myStateMachine != null) {
            Element element = getElement(myStateMachine.getStateMachine().
                    getState());
            if (element != null) {
                MyRule myRule = (MyRule) element.getData();
                bringGuiImpactOfStateMachine(myStateMachine.getStateMachine(),
                        myRule, controleStateMachineRule(myRule));
            }
            highlightDiagramSmState(myStateMachine.getStateMachine(),
                    uniqueMyRuleMap);
        } else {
            resetTriggers();
        }
    ...
    public String startProcess() throws WorkflowExcepiton {
        ...
        try {
            ...
            myStateMachine = MyStateMachineFactory
                    ...
                    buildStateMachine(myForm.getWorkflowSteps(), myForm.
                            getWorkflowStartStep());

            String state = myStateMachine.getStateMachine().
                    getState();
            ...
            if (element == null) {
                dialogController.showPopupError(String.format(
                        "element on state %s is resolved to null ", state));
                return null;
            }
            ...
            saveSmStateOnObject(myStateMachine.getStateMachine(),
                    myCommandResult, "START");

            bringGuiImpactOfStateMachine(myStateMachine.getStateMachine(),
                    myRule, myCommandResult);
    ...
    public String fireProcess() {
        ...
        if (myStateMachine != null) {
            ...
            for (String t : myStateMachine.getStateMachine().
                    getPermittedTriggers()) {

                if (trigger.equals(t) && myStateMachine.getStateMachine().
                        canFire(t)) {

                    TriggerWithParameters1 twp1 = (TriggerWithParameters1) myStateMachine.
                            getStateMachineConfig().
                            getTriggerConfiguration(trigger);

                    if (twp1 == null) {
                        twp1 = new TriggerWithParameters1<>(t,
                                WorkFlowCtrl.class);
                    }
                    ...
                    MyRule myRule = (MyRule) getElement(myStateMachine.
                            getStateMachine().
                            getState()).
                            getData();

                    MyCommandResult myCommandResult = controleStateMachineRule(
                            myRule);

                    saveSmStateOnObject(myStateMachine.getStateMachine(),
                            myCommandResult, trigger);

                    bringGuiImpactOfStateMachine(myStateMachine.
                            getStateMachine(), myRule, myCommandResult);
                }
            }
        }
    ...
    private MyCommandResult controleStateMachineRule(MyRule myRule) {
        Document commandResult = mongoDbUtil.runCommand("test", myRule.
                getControlFunc().
                getCode(), crudObject, loginController.getRolesAsSet());
    ...
    public void highlightDiagramSmState(StateMachine<String, String> sm,
        ...
        elementMap.get(sm.getState()).
                setStyleClass("ui-diagram-element-entry");
    }
    ...
    public void createTriggers(MyMap myMap) {
        ...
        if (myStateMachine != null) {
            triggers = myStateMachine.getStateMachine().
                    getPermittedTriggers();
        }
    }
    ...
    public MyCommandResult saveObjTrig(MyCommandResult smControlResult) {
        ...
        if (myStateMachine != null) {
            ...
            crudObject.put(SM_STATE, myStateMachine.getStateMachine().
                    getState());
            ...
            if (smControlResult == null) {
                ...
                crudObject.put(SM_TRIGGER,
                        crudObject.isNew() ? "CREATE_RECORD" : "UPDATE_RECORD");
    ...
    private void saveSmStateOnObject(StateMachine stateMachine,
        ...
        mongoDbUtil.updateOne(myForm.getDb(), myForm.getTable(), updateFilter,
                update);
        mongoDbUtil.updatePush(myForm.getDb(), myForm.getTable(), updateFilter,
                new Document("wfHistory", update));
    }
    ...
    private void bringGuiImpactOfStateMachine(
        ...
        if (!mcr.getResult()) {
            ...
            FacesMessage facesMessageRequired = new FacesMessage(
                    FacesMessage.SEVERITY_ERROR, err, "*");
            FacesContext.getCurrentInstance().
                    addMessage(null, facesMessageRequired);

        }
        if (myRule.isLast()) {
            getElement(stateMachine.getState()).
                    setStyleClass("ui-diagram-element-exit");
        }

    }

    private MyRule getMyRule(StateMachine<String, String> stateMachine) {
        return (MyRule) uniqueMyRuleMap.get(stateMachine.getState()).
                getData();
    }
    ...
    private void renderFields(StateMachine<String, String> sm) {
        ...
        for (String fieldKey : currentRule.getFields().
                keySet()) {
            myForm.getField(fieldKey).
                    setRendered(true);
            myForm.getField(fieldKey).
                    createSelectItems(filter,
                            crudObject,
