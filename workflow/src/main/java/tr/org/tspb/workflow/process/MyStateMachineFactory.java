package tr.org.tspb.workflow.process;
...
public class MyStateMachineFactory {
    ...
    public static MyStateMachineFactory instance(WorkFlowCtrl twoDimModifyCtrl,
    ...
    public MyStateMachine buildStateMachine(List<MyRule> myRules, String state) {
        ...
        StateMachine<String, String> stateMachine = new StateMachine(state,
                stateMachineConfig);
        ...
        for (final MyRule myRule : myRules) {
            ...
            StateConfiguration stateConfiguration = stateMachineConfig.
                    configure(myRule.getCode());

            for (final MyTransition myTransition : myRule.getTransitions()) {

                String expandedTrigger = myRule.getCode() + "_" + myTransition.
                        getTrigger();

                TriggerWithParameters1 twp1 = stateMachineConfig.
                        setTriggerParameters(expandedTrigger, WorkFlowCtrl.class);

                stateConfiguration
                        .permitIf(expandedTrigger, myTransition.getState(),
                                new MyFuncBoolean(myTransition.getTrigger(),
                                        myRule.getControlFunc().
                                                getCode()));
                ...
                stateMachineConfig.configure(nextRuleCode).
                        onEntry(new OnEntryAction(nextRule.getOnEntry().
                                getCode(), expandedTrigger)).
                        onEntryFrom(twp1, new OnEntryFromAction(nextRule.
                                getOnEntryFrom().
                                getCode(), expandedTrigger), WorkFlowCtrl.class);
...
//            System.out.println(result);
    ...
    private class OnEntryFromAction implements Action1<WorkFlowCtrl> {
        ...
        public void doIt(WorkFlowCtrl t) {
            Document result = mongoDbUtil.runCommand("test", mongoCode,
                    workFlowCtrl.getCrudObject(), trigger);
            System.out.println(result);
        }
    ...
    private class MyFuncBoolean implements FuncBoolean {
        ...
        @Override
        public boolean call() {
            Document commandResult = mongoDbUtil.runCommand("test", code,
                    workFlowCtrl.getCrudObject(), workFlowCtrl.
                    getLoginController().
                    getRolesAsSet());
            return getControlResult(commandResult, trigger).
                    getResult();
        }
    ...
    private MyCommandResult getControlResult(Document commandResult,
        ...
        if (retval instanceof Document) {
            ...
            result = calculatedTriggers == null ? false : calculatedTriggers.
                    contains(trigger);
