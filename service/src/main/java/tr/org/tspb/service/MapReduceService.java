...
public class MapReduceService implements Serializable {
    ...
    public void init(Map<String, Object> filter, FmsForm myForm) {
        this.filter = filter;
        this.configCollection = myForm.getMyProject().
                getConfigTable();
    ...
    public Map<Map, Serializable> mapReduceSnapshot(Map cashedKey,
            FmsForm myForm)
            throws NullNotExpectedException, MongoOrmFailedException,
            MoreThenOneInListException, FormConfigException {
        ...
        for (MyField myField : myForm.getZetDimension()) {
            ...
            if (!cashedKey.containsKey(field)) {
                throw new RuntimeException(myField.getKey().
                        concat(" : z-dimension key missed on mapreduce filter"));
            }
        }
    ...
    private Map<Map, Serializable> mapReduce(Map cashedKey, String collection)
        ...
        FmsForm myForm = appScopeSrvCtrl.
                getFormDefinitionByKey(configCollection, collectionKey, filter);

        Bson concluededQuery = Filters.and(new Document(cashedKey), Filters.eq(
                FORMS, myForm.getKey()));

        List<Document> cursor = mongoDbUtil.
                find(db, collection, concluededQuery);

        List<DocumentRecursive> list = repositoryService.cursorToList(cursor,
                myForm);
        ...
        MyMapReduce myMapReduce = new MyMapReduce(initParams) {
            ...
            public void map(Map recordMap) {
                ...
                for (Object y : (Iterable<? extends Object>) initParams.get("y")) {
                    for (Object x : (Iterable<? extends Object>) initParams.get(
                            "x")) {
            ...
            @Override
            public Serializable reduce(Map key, List<Serializable> values)
