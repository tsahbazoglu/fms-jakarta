...
public class DownloadService extends CommonSrv {
    ...
    private void init() {
        ...
        searcheDBObject = new Document()
                .append(selectedForm.getLoginFkField(), searchedMap.get(
                        selectedForm.getLoginFkField())).
                append(PERIOD, searchedMap.get(PERIOD));

        if (searcheDBObject.get(selectedForm.getLoginFkField()) == null && loginController.
                isUserInRole(selectedForm.getMyProject().
                        getAdminAndViewerRole())) {
            searcheDBObject.remove(selectedForm.getLoginFkField());
        }

        if (searcheDBObject.get(PERIOD) == null && loginController.isUserInRole(
                selectedForm.getMyProject().
                        getAdminAndViewerRole())) {
    ...
    public void createExcellFile() {
        ...
        if (formService.getMyForm().
                getExcelFormat() != null) {
            sort = (Document) formService.getMyForm().
                    getExcelFormat().
                    get(SORT);
            limit = ((Number) formService.getMyForm().
                    getExcelFormat().
                    get("limit")).intValue();
        }
        ...
        try (XSSFWorkbook xssfWorkbook = new XSSFWorkbook()) {
            ...
            List<String> columnList = new ArrayList<>(formService.getMyForm().
                    getFieldsKeySet());
            if (formService.getMyForm().
                    getExcelFormat() != null) {
                columnList = (List<String>) formService.getMyForm().
                        getExcelFormat().
                        get("keys");
            }
            ...
            for (String key : columnList) {
                MyField field = formService.getMyForm().
                        getField(key);
                if (field.getItemsAsMyItems() != null && field.
                        getItemsAsMyItems().
                        getView() != null) {
                    for (String columnName : field.getItemsAsMyItems().
                            getView()) {
                        row.createCell(y++).
                                setCellValue(field.getDbo().
                                        get(columnName) == null ? columnName : field.
                                        getDbo().
                                        get(columnName).
                                        toString());
                    }
            ...
            for (Document data : cursor) {
                ...
                for (String key : columnList) {
                    ...
                    if (keyValue instanceof ObjectId && !MONGO_ID.equals(key)) {
                        if (field != null) {
                            ...
                            if (tempCache.get(keyValueObjectId) == null) {
                                ...
                                Document record = mongoDbUtil.findOne(
                                        myDb == null ? formService.getMyForm().
                                                        getDb() : myDb,
                                        myColl,
                                        new Document(MONGO_ID, keyValueObjectId));

                                if (record == null) {
                                    tempCache.put(keyValueObjectId, Arrays.
                                            asList("NO DATA ERROR"));
                                } else {
                                    ...
                                    for (String columnName : field.
                                            ...
                                            getView()) {
                                        names.add(
                                                record.get(columnName) == null ? "NO NAME" : record.
                                                get(columnName).
                                                toString());
                                    }
                        ...
                        keyValue = getTempCacheBsonconverter(keyValue.toString(),
                                field);
                    }
                    ...
                    if (keyValue instanceof List) {
                        ...
                        for (String strValue : (List<String>) keyValue) {
                            ...
                            cell.setCellValue(
                                    keyValue == null ? "no data" : strValue);
                        }

                    } else {
                        XSSFCell cell = row.createCell(y++);
                        cell.setCellValue(
                                keyValue == null ? "no data" : keyValue.
                                                toString());
            ...
            fileName = SIMPLE_DATE_FORMAT__5.format(new Date()).
                    concat("_").
            ...
            try {
                fileOut = new FileOutputStream(baseService.getProperties().
                        getTmpDownloadPath().
                        concat(getFileName()).
                        concat(EXTENSION_XLSX), true);
    ...
    public StreamedContent getFile() {
        ...
        String downloadFileName = formService.getMyForm().
                getName().
                concat(EXTENSION_XLSX);
        String downloadFilePath = baseService.getProperties().
                getTmpDownloadPath().
                concat(getFileName()).
                concat(EXTENSION_XLSX);
        ...
        try {
            ...
            file = DefaultStreamedContent.builder().
                    ...
                    contentType("application/octet-stream;charset=utf-8") // .stream(() -> FacesContext.getCurrentInstance().getExternalContext().getResourceAsStream(downloadFilePath))
                    .
    ...
    public void downloadExcellFile() {
        try {
            ...
            String encodedFileName = URLEncoder.encode(formService.getMyForm().
                    getName().
                    concat(EXTENSION_XLSX), "UTF-8");

            ec.setResponseHeader("Content-Disposition",
                    "attachment; filename*=UTF-8''".concat(encodedFileName));
    ...
    public String downloadCsvFile() {
        ...
        if (!isExcellFormatDefined) {
            if (searchedMap.get(PARENT_VALUE) != null) {
                searcheDBObject.append(PARENT_VALUE, searchedMap.get(
                        PARENT_VALUE));
            }
        }

        List<DocumentRecursive> list = repositoryService.findList(selectedForm,
                searcheDBObject, 20000);
        ...
        for (DocumentRecursive tdo : list) {
            for (MyField column : objectsColumnDataModel) {
                ...
                if (value == null) {
                    sw.append("");
                } else if (column.getMyconverter() instanceof MoneyConverter || "#.###.###,##".
                        equals(converterFormat)) {
                    ...
                    if (PARENT_VALUE.equals(thekey)) {
                        ...
                        if (isExcellFormatDefined) {
                            ...
                            sw.append(";");
                            sw.append(((Document) value).getString(
                                    "abstractOrder"));
                            sw.append(";");
                            sw.append(((Document) value).
                                    getString("detailOrder"));
                        }
                    } else {
                        sw.append(((Document) value).getString(NAME));
                    }
    ...
    public String downloadCsvFilePivot(List<MyField> iksDimension,
        ...
        Document period = mongoDbUtil.findOne(formService.getMyForm().
                getDb(),
                COMMON, new Document(MONGO_ID, filterService.
                        getPivotFilterCurrent().
                        get(PERIOD)));

        Document template = mongoDbUtil.findOne(formService.getMyForm().
                getDb(),
                DATA_BANK_TEMPLATE, new Document(MONGO_ID, filterService.
                        getPivotFilterCurrent().
                        get(TEMPLATE)));
        ...
        for (MyField coordinateX : igrekDimension) {
            header.append(coordinateX.getName().
                    concat(";"));
        }
        header.append("\n");
        sw.append(header.toString());
        ...
        for (MyField coordinateX : iksDimension) {
            String row = "";
            row += (period == null ? "-" : period.get(NAME).
                    toString()).concat(";");
            row += (template == null ? "-" : template.get(NAME).
                    toString()).concat(";");

            row += coordinateX.getName().
                    concat(";");
            ...
            for (MyField coordinateY : igrekDimension) {
                ...
                CellMultiDimensionKey key = new CellMultiDimensionKey(
                        coordinates);
                CustomOlapHashMap customOlapHaspMap = mapMultiDimension.get(key).
                        get(0);
        ...
        if (Boolean.TRUE.equals(formService.getMyForm().
            ...
            period = mongoDbUtil.findOne(formService.getMyForm().
                    getDb(), COMMON,
                    new Document(MONGO_ID,
                            filterService.getPivotFilterCurrent().
                                    get(PERIOD)));

            template = mongoDbUtil.findOne(formService.getMyForm().
                    getDb(), DATA_BANK_TEMPLATE,
                    new Document(MONGO_ID,
                            filterService.getPivotFilterCurrent().
                                    get(TEMPLATE)));
            ...
            for (MyField coordinateX : iksDimension) {
                ...
                for (MyField coordinateY : igrekDimension) {
                    ...
                    CellMultiDimensionKey key = new CellMultiDimensionKey(
                            coordinates);
                    CustomOlapHashMap customOlapHaspMap = mapMultiDimensionHistory.
                            get(key).
                            get(0);
                    Object value = customOlapHaspMap.getValue();
                    if (value != null) {
                        ...
                        if (converter != null //
                                ...
                                && converter instanceof MoneyConverter) {
                            Locale locale = new Locale("tr", "TR");
                            NumberFormat toView = NumberFormat.getInstance(
                                    locale);//
        ...
        if (formService.getMyForm().
                getExcelFormat() != null) {
            excelFormat = formService.getMyForm().
                    getExcelFormat().
                    getString("template");
        }
        ...
        httpSession.setAttribute(UYS_DOWNLOAD_ROLES, loginController.
                getRoleMap().
                keySet());
