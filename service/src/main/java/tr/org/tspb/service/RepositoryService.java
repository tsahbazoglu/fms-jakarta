...
public class RepositoryService implements Serializable {
    ...
    public Document expandCrudObject(FmsForm fmsForm, Document operatedObject) {
        ...
        for (String key : operatedObject.keySet()) {
            ...
            if (object instanceof String //
                ...
                if (object.toString().
                        startsWith("[")) {
                    List<String> list = gsonConverter.
                            fromJson(object.toString(), gsonType);
            ...
            if (object instanceof DualListModel) {
                operatedObject.put(key, ((DualListModel) object).getTarget());
            }
            ...
            if (ComponentType.chips.name().
                ...
                if (list != null) {
                    for (SelectItem selectItem : list) {
                        targetList.add((ObjectId) selectItem.getValue());
                    }
                }
                operatedObject.put(key, targetList);
            }
        }
    ...
    public List<Map<String, String>> findGridFsFileList(
            ObjectId myCommonRecordID) {
        ...
        if (myCommonRecordID != null) {
            ...
            for (GridFSFile gridFSDBFile : files) {
                ...
                String idAsStr = gridFSDBFile.getObjectId().
                        toString();
                fileInfoPresent.put(FILE_ID, idAsStr);
        ...
        for (GridFSFile gridFSDBFile : files) {
            ...
            fileInfoPresent.put(FILE_STATE,
                    "Bu ek henüz ilişkilendirilmemiş. Günlük Sistem Bakımında otomotik silinecek. İlişkilendirmek için 'Kaydet' tuşuna basınız");
    ...
    public List<Map<String, String>> findGridFsFileList(FmsForm myForm) {
        ...
        for (GridFSFile gridFSDBFile : files) {
            ...
            fileInfoPresent.put(FILE_ID, gridFSDBFile.getObjectId().
                    toString());
            fileInfoPresent.put("METADA_USERNAME", (String) gridFSDBFile.
                    getMetadata().
                    get("username"));
            fileInfoPresent.put(FILE_NAME, gridFSDBFile.getFilename());
            fileInfoPresent.put(FILE_SIZE, String.valueOf(gridFSDBFile.
    ...
    public List<ModuleItem> createModuleList() throws NullNotExpectedException,
            FormConfigException {
        ...
        Bson filter = Filters.and(
                Filters.elemMatch(ACCESS_CONTROL, Filters.eq(DOLAR_IN,
                        listOfRoles)),
                Filters.eq(TYPE, NODE),
                Filters.eq(ENABLED, true));

        List<Document> projectsCursor = mongoDbUtil.find(CONFIG_DB,
                CFG_TABLE_PROJECT);

        //add accessible modules in each project
        for (Document project : projectsCursor) {

            MyProject myProject = appScopeSrvCtrl.getProject(project.getString(
                    FORM_KEY), project);

            List<Document> listOfModules = mongoDbUtil.find(CONFIG_DB,
                    myProject.getConfigTable(), filter);
    ...
    public List<FormItem> findModuleForms(ModuleItem moduleItem) {
        ...
        Document query = new Document()
                .append(UPPER_NODES.concat(DOT).
                        concat(moduleItem.getModuleKey()), new Document(
                        DOLAR_EXISTS, true)).
                append(ACCESS_CONTROL, new Document(DOLAR_IN, loginController.
                        getRolesAsList()));

        List<Document> documents = mongoDbUtil.find(CONFIG_DB, moduleItem.
                getProject().
                getConfigTable(), query);
        ...
        for (Document dBObject : documents) {
            ...
            if (formDisabled instanceof Document && !loginController.
                    isUserInRole(((Document) formDisabled).getString(
                            NOT_ON_USER_ROLE))) {
                continue;
            }
    ...
    public List<FormItem> findModuleFormsSchemaVersion110(ModuleItem moduleItem) {
        ...
        Bson filter = Filters.and(
                Filters.or(
                        Filters.eq(FmsForm.SCHEMA_VERSION,
                                FmsForm.SCHEMA_VERSION_110),
                        Filters.eq(FmsForm.SCHEMA_VERSION,
                                FmsForm.SCHEMA_VERSION_111)),
                Filters.eq(UPPER_NODES, moduleItem.getModuleKey()),
                Filters.eq(ACCESS_CONTROL, new Document(DOLAR_IN,
                        loginController.getRolesAsList())));

        List<Document> documents = mongoDbUtil.find(CONFIG_DB, moduleItem.
                getProject().
                getConfigTable(), filter);
    ...
    public ModuleItem getModuleItem(String configTable, String upperNodeKey) {
        ...
        Document mongoModuleForm = mongoDbUtil.findOne(CONFIG_DB, configTable,
                new Document(FORM_KEY, upperNodeKey));
    ...
    public String createWelcomePage(String welcomepage) {
         ...
         */
        List<GridFSFile> gridFSDBFiles = mongoDbUtil.findFiles("welcomedb",
                welcomepage);

        if (!gridFSDBFiles.isEmpty()) {

            GridFSFile gridFSFile = gridFSDBFiles.get(0);
    ...
    public DatabaseUser getDatabaseUser(MyProject myProject,
        ...
        Document memberDbo = mongoDbUtil.findOne(loginDB, loginTable,
                new Document(usernameField, loginController.
                        getLoggedUserDetail().
                        getUsername()));

        if (memberDbo == null) {
            throw new RuntimeException(String.format(
                    "no detail info regarding to logged user",
    ...
    public MyProject getMyProject(String projectKey) throws
            NullNotExpectedException, FormConfigException {
        return ogmCreator.getMyProject(
                mongoDbUtil.findOne(CONFIG_DB, CFG_TABLE_PROJECT, Filters.eq(
                        FORM_KEY, projectKey)),
    ...
    public Map getRecord(String db, String table, Object showDetailObject) {
        Document record = mongoDbUtil.findOne(db, table, new Document(MONGO_ID,
                showDetailObject));
        return record;
    }
    ...
    public Map oneAsUserOrDefault(String db, String table,
        ...
        if (map == null) {
            ...
            map.put(DEFAULT_LOGIN_FK_FILED_NAME, loginController.
                    getLoggedUserDetail().
                    getDbo().
                    getObjectId());
        }
    ...
    public Map oneAsUser(String db, String table, Map<String, Object> searchMap) {
        ...
        dBObject.put(DEFAULT_LOGIN_FK_FILED_NAME, loginController.
                getLoggedUserDetail().
                getDbo().
                getObjectId());
    ...
    public Map one(String db, String table, Bson searchMap) {
        return mongoDbUtil.findOne(db, table, searchMap);
    }
    ...
    public long count(String db, String table, Bson filter) {
        return mongoDbUtil.count(db, table, filter);
    }
    ...
    public Map oneById(String anketdb, String anket_masak, String idStr) {
        ...
        return mongoDbUtil.
                findOne(anketdb, anket_masak, new Document(searchMap));
    }

    public List<Map> list(String db, String coll, Map query) {
        List<Document> documents = mongoDbUtil.find(db, coll,
                new Document(query));
    ...
    public List<Map> listAsUser(FmsForm selectedForm, Map searchMap) {
        ...
        List<Document> documents = mongoDbUtil.find(selectedForm.getDb(),
                selectedForm.getTable(), new Document(dBObject));
    ...
    public List<Map> findSkipLimitAsLoggedUser(String db, String table,
        ...
        if (!loginController.isUserInRole("ionadmin")) {
            searchedMap.put(DEFAULT_LOGIN_FK_FILED_NAME, userDetail.getDbo().
                    getObjectId());
        }
        List<Document> documents = mongoDbUtil
                .findSkipLimit(db, table, new Document(searchedMap), startRow,
                        maxResults);
    ...
    public void updateManyMyLicenseAsUser(FmsForm myForm, Map searchForUpdate,
        ...
        if (myLicense.getAsDBObject().
                get(MONGO_ID) != null) {
            mongoDbUtil
                    .updateMany(myForm, new Document(searchForUpdate),
                            myLicense.getAsDBObject().
                                    append(JOIN_ID, joinId));
        } else {
            mongoDbUtil
                    .updateMany(myForm, new Document(searchForUpdate),
                            myLicense.getAsDBObject().
    ...
    public void updateManyMyRecordAsUser(FmsForm myForm, Map searchForUpdate,
        ...
        if (myRecord.getCrudMap().
                get(MONGO_ID) != null) {
            mongoDbUtil.updateMany(myForm, new Document(searchForUpdate),
                    myRecord.getAsDBObject().
                            append(JOIN_ID, joinId));
        } else {
            mongoDbUtil
                    .updateMany(myForm, new Document(searchForUpdate), myRecord.
                            getAsDBObject().
                            append(JOIN_ID, joinId), new UpdateOptions().upsert(
                            true));
        }

    }

    public void createFile(FmsForm selectedForm, MyRecord myRecord) throws
            IOException {
        ...
        metadata.put("username", loginController.getLoggedUserDetail().
                getUsername());

        String uploadTable = baseService.getProperties().
                getUploadTable();

        String fileName = myRecord.getUploadedFile().
                getFileName();

        try (InputStream inputStream = myRecord.getUploadedFile().
                getInputStream()) {

            ObjectId fileId = mongoDbUtil.createFile(uploadTable, fileName,
    ...
    public MyFile findFileAsMyFile(String ion_uploaded_files, String objectID)
            throws IOException {
        return mongoDbUtil.findFileAsMyFile(baseService.getProperties().
                getUploadTable(), new ObjectId(objectID));
    }
    ...
    public String sendForm(FmsForm fmsForm, ObjectId period, Document filterDoc,
        ...
        try {
            ...
            successMessage = String.format(successMessage,
                    SIMPLE_DATE_FORMAT__3.format(new Date()), periodName);

            String myActionType = tagActionsAction.getEnableResult().
                    getMyActionType();
            String javaFunc = tagActionsAction.getEnableResult().
                    getJavaFunc();
            String code = tagActionsAction.getEnableResult().
                    getMyaction();

            List<TagActionsAction.Operation> operaiotns = tagActionsAction.
                    getOperations();

            if (operaiotns != null && !operaiotns.isEmpty()) {
                callAdditionalAction(fmsForm, filterService.
                        getTableFilterCurrent(), tagActionsAction, crudObject);
            ...
            if (myActionType == null) {
                if (code == null) {
                    ...
                    throw new Exception(
                            "Bu olay üzerinde eylem tanımlı değil.<br/>Sistem yöneticisi ile iletişime geçiniz.");
                } else {
                    Document mySearchObject = expandCrudObject(fmsForm,
                            filterDoc);
                    ...
                    mongoDbUtil.
                            runCommand(fmsForm.getDb(), code, mySearchObject,
                                    null);
                    return successMessage;
                }
            } else if ("java".equals(myActionType)) {
                Method method = this.getClass().
                        getMethod(javaFunc, new Class[]{});
    ...
    public Document findRiskTedbir(ObjectId loginObjectId, String riskCode) {
        return mongoDbUtil.findOne("anketdb", "masakRisk",
                Filters.and(
                        Filters.eq("memberId", loginObjectId),
                        Filters.eq("risk-code", riskCode)));
    }

    public List<Document> findRisks(ObjectId loginObjectId) {
        return mongoDbUtil.find("anketdb", "masakRisk",
                Filters.and(Filters.eq("memberId", loginObjectId)));
    }

    public void saveRisk(Document risk, ObjectId loginObjectId) {
        risk.append("memberId", loginObjectId);
        ObjectId recordId;
        if ((recordId = risk.get("_id", ObjectId.class)) != null) {
            mongoDbUtil.updateOne("anketdb", "masakRisk", Filters.eq("_id",
                    recordId), risk);
        } else {
            mongoDbUtil.upsertOne("anketdb", "masakRisk", Filters.and(
                    Filters.eq("memberId", loginObjectId),
                    Filters.eq("risk-code", risk.getString("risk-code"))), risk);
        }
    }
