...
import tr.org.tspb.ejb.shared.SharedValues;

/**
 *
 * @author Telman Şahbazoğlu
 */
@MessageDriven(mappedName = MessageDriverBeanEmailQueueBean.JMS_EMAIL_QUEUE,
        activationConfig = {
...
public class MessageDriverBeanEmailQueueBean implements MessageListener {
    ...
    @Resource(lookup = CUSTOM_RESOURCE_MAIL_EXTERNAL,
            type = jakarta.mail.Session.class)
    private Session sessionTspakbOutsideMail;

    @Resource(lookup = CUSTOM_RESOURCE_MAIL_INTERNAL,
            type = jakarta.mail.Session.class)
    ...
    public void onMessage(Message message) {
        ...
        if (message instanceof ObjectMessage) {
            ...
            try {
                EmailData emailData = (EmailData) ((ObjectMessage) message).
                        getObject();
    ...
    private void sendEmail(Transport transport, Session session,
            EmailData emailData) {
        ...
        try {
            ...
            if (emailData.getBccRecipients() != null) {
                msg.setRecipients(RecipientType.BCC, InternetAddress.parse(
                        emailData.getBccRecipients(), false));
            }
            ...
            try {
                ...
                msg.setSubject(MimeUtility.encodeText(emailData.getSubject(),
                        "UTF-8", "Q"));
            ...
            if (attachmentBytes != null) {
                ...
                messageBodyPart.setContent(emailData.getMessage(),
             ...
             */
            if (multipart == null) {
                msg.setContent(emailData.getMessage(),
                        "text/html;charset=\"UTF-8\"");
            } else {
            ...
            try {
                JAXBContext contextEmailData = JAXBContext.newInstance(
                        EmailData.class);
                Marshaller marshallEmailData = contextEmailData.
                        createMarshaller();
                marshallEmailData.setProperty(
            ...
            logger.log(Level.SEVERE, "Could not send an email to :".concat(
                    emailData.getRecipients()));
            logger.log(Level.SEVERE, null, ex);
        } finally {
            try {
                if (transport != null) {
