...
public class FreeDesigner implements Serializable {
    ...
    public List<PlainRecord> completeMethod(String query) {
        ...
        if (query != null && !query.isEmpty()) {
            list = repositoryService
                    .findList("uysdb", "common", null,
                            Filters.regex(NAME, query.toUpperCase(new Locale(
                                    "tr", "TR")), "i"), 10);
        } else {
            list = repositoryService.findList("uysdb", "common", null,
                    new Document(), 10);
        }
        ...
        for (DocumentRecursive documentRecursive : list) {
            plainRecords
                    .add(PlainRecordData
                            .getPlainRecord(new SelectItem(documentRecursive.
                                    get(MONGO_ID), (String) documentRecursive.
                                    get(NAME))));
        }
        return plainRecords;
    }
        ...
        if (myForm != null && myForm.getKey() != null) {
            try {
                ...
                List<Map> cursor = repositoryService
                        .findSkipLimitAsLoggedUser(selectedForm.getDb(), myForm.
                                getTable(), searchedMap, startRow, maxResults);
    ...
    public MyRecord getRowData(String rowKey) {
        ...
        return new MyRecord(loginController.getRoleMap(), loginController.
                getLoggedUserDetail(),
...
//        myRecords.setRowCount(3000000);
    ...
    public void init(FmsForm selectedForm) {
        ...
        if (loginController.isUserInRole(selectedForm.getMyProject().
                getAdminAndViewerRole())) {
            refreshLazy();
            Map searchedMap = new HashMap();
            searchedMap.put(FIELD_GROUP, FIELD_GROUP_COMMON);
            this.myRecords.setRowCount((int) repositoryService.count(
                    selectedForm.getDb(), selectedForm.getTable(), searchedMap));
        } else {
            this.myRecord = findOne();
            if (selectedForm.getFieldsKeySet().
                    contains("ekler")) {
        ...
        } else if (loginController.isUserInRole(selectedForm.getMyProject().
                getUserRole())) {
    ...
    public String showEimza() {
        ...
        if (listOfCruds.isEmpty()) {
            dialogController.showPopup("Uyarı",
    ...
    private void refreshUploadedFileList() {
        if (myRecord != null) {
            listFileData = repositoryService.findGridFsFileList(myRecord.
                    getMongoIdAsString());
        }
    }
    ...
    private List<MyLicense> wrap(List<MyLicense> list) {
        ...
        if (loginController.isUserInRole("pyuser")) {
            for (MyLicense myLicense : MyLicense.getLicenceParentPys().
                    values()) {
                wrappedListOfMyLicense.add(new MyLicense(myLicense, roles));
            }
        } else {
            for (MyLicense myLicense : MyLicense.getLicenceParent().
                    values()) {
                wrappedListOfMyLicense.add(new MyLicense(myLicense, roles));
            }
        }
    ...
    public void saveAsLoggedUser(MyRecord myRecord) {
        ...
        String memberUserName = loginController.getLoggedUserDetail().
                getUsername();
        ...
        for (MyLicense myLicense : myRecord.getListOfLicense()) {
            if (myLicense.getSaveToDb()) {
                ...
                searchForUpdate.put(MyLicense.LICENSE_KEY, myLicense.
                        getLicense());
                searchForUpdate.put(FIELD_GROUP, FIELD_GROUP_LICENSE);

                myLicense.setAdminMetaData(
                        loginController.getLoggedUserDetail().
                                getUsername(), new Date(), sessionId);

                repositoryService.updateManyMyLicenseAsUser(selectedForm,
                        searchForUpdate, myLicense, joinId);
            }
        }
        ...
        Document uysAdditionalMetaData = (Document) myRecord.getCrudMap().
                get(ADMIN_METADATA);
        if (uysAdditionalMetaData == null) {
            uysAdditionalMetaData = new Document();
            myRecord.getCrudMap().
                    put(ADMIN_METADATA, uysAdditionalMetaData);
            uysAdditionalMetaData.put(CREATE_USER, loginController.
                    getLoggedUserDetail().
                    getUsername());
            ...
            uysAdditionalMetaData.put(UPDATE_USER, loginController.
                    getLoggedUserDetail().
                    getUsername());
        }

        repositoryService.
                updateManyMyRecordAsUser(selectedForm, searchForUpdate, myRecord,
                        joinId);

        // provide uploaded file relation
        if (selectedForm.isHasAttachedFiles()) {
            Map dbo = repositoryService.one(selectedForm.getDb(), selectedForm.
                    getTable(), searchForUpdate);
    ...
    public MyRecord findOne() {
        ...
        Map next = repositoryService.oneAsUserOrDefault(selectedForm.getDb(),
                selectedForm.getTable(), searchMap1);

        MyRecord record = new MyRecord(loginController.getRoleMap(),
                loginController.getLoggedUserDetail(), next, selectedForm,
                appScopeSrvCtrl.memberCacheNameById());
    ...
    public List<String> getSelectedFormFieldKeySetAsList() {
        ...
        for (String key : selectedForm.getFieldsKeySet()) {
            if (loginController.isUserInRole(selectedForm.getField(key).
                    getAccesscontrol())) {
                list.add(key);
            }
        }
        Collections.sort(list, new Comparator<String>() {
            ...
            public int compare(String t, String t1) {
                String nameCurrent = selectedForm.getField(t).
                        getOrder().
                        toString();
                String nameNext = selectedForm.getField(t1).
                        getOrder().
                        toString();
