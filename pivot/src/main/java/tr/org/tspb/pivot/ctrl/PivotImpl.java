...
import tr.org.tspb.datamodel.dao.MyFieldComparator;
...
public abstract class PivotImpl implements Serializable, PivotApi {
    ...
    private List<MyField> createDimensionFieldItems(FmsForm myForm,
        ...
        List<MyField> listOfMap = appScopeSrvCtrl.cacheAndGetDimensionItems(
                cacheKey);
        ...
        Object collectionName = myField.getItemsAsMyItems().
                getTable();

        if (collectionName instanceof Code) {
            Document commandResult = mongoDbUtil
                    .runCommand(myForm.getDb(), ((Code) collectionName).
                            getCode(), enrichSearchObject, null);
            collectionName = commandResult.get(RETVAL);
        }

        Document projection = myField.getItemsAsMyItems().
                getQueryProjection();
        if (projection != null) {
        ...
        for (Document dbObject : cursor) {
            ...
            if (FORM_DIMENSION.equals(myField.getNdType()) && (code == null || code.
            ...
            for (String keyy : myField.getItemsAsMyItems().
                    getView()) {
                name.append("\t|\t");
    ...
    public void refreshRowData(FmsForm myForm) {
        ...
        Document dbo = mongoDbUtil.findOne(myForm.getDb(), myForm.getTable(),
                new Document(getFilter()).append(FORMS, FIELDS_ROW));
...
//            }
        ...
        if (FmsFieldItems.ItemType.list.equals(myField.getItemsAsMyItems().
            ...
            for (Object itemValue : docItems) {
                if (itemValue instanceof Document) {
                    listSelectItems.add(new SelectItem(((Document) itemValue).
                            get(CODE), (String) ((Document) itemValue).get(NAME)));
                }
            }
        }
    ...
    public void prepareJsfComponentMap(FmsForm inodeMyForm) {
        ...
        for (String key : inodeMyForm.getFieldsRowKeys()) {
            ...
            Boolean calculateOnCrudView = fieldStructure.
                    getCalculateOnCrudView();
                ...
                if (fieldStructure.getMyconverter() instanceof MoneyConverter) {
                    ...
                    for (SelectItem selectItem : items) {
                        itemMap.put(selectItem.getValue().
                                toString(), selectItem.getLabel());
                    }
                    ((SelectOneObjectIdConverter) myconverter).setItemMap(
                            itemMap);
            ...
            if (rendered instanceof Document && !loginController.isUserInRole(
                    ((Document) rendered).get(ON_USER_ROLE))) {
                continue;
            }

            componentMap.put(key, new HashMap());
            componentMap.get(key).
                    put(LABEL, fieldStructure.getName());
            if (fieldStructure.getStyle() != null) {
                componentMap.get(key).
                        put(STYLE, fieldStructure.getStyle());
            }

            componentMap.get(key).
                    put(REQUIRED, Boolean.TRUE.equals(fieldStructure.
                            isRequired()));
            componentMap.get(key).
                    put(RENDERED, Boolean.TRUE.equals(fieldStructure.
                            isRendered()));
            componentMap.get(key).
                    put(COMPONENTTYPE, fieldStructure.getComponentType());
            componentMap.get(key).
                    put(ITEMS, items);
            componentMap.get(key).
                    put(CONVERTER, myconverter);
            componentMap.get(key).
                    put(CONVERTER_INSTANCE, fieldStructure.
                            getConverterInstance());
            componentMap.get(key).
                    put(CONVERTER_FORMAT, converterFormat);
            componentMap.get(key).
                    put(CALCULATE_ON_SAVE, calculateOnSave);
            componentMap.get(key).
                    put(CALCULATE_ON_CRUD_VIEW, calculateOnCrudView);
            componentMap.get(key).
                    put(IS_AJAX, Boolean.TRUE.equals(fieldStructure.getAjax().
                            isEnable()));
            if (fieldStructure.getAjax().
                    getEffectedKeys() != null) {
                String ajaxUpdate = "";
                for (String ajaxKey : fieldStructure.getAjax().
                        getEffectedKeys()) {
                    ajaxUpdate += ":crud2dForm:label_".concat(ajaxKey).
                            concat(COMMA);
                    ajaxUpdate += ":crud2dForm:component_".concat(ajaxKey).
                            concat(COMMA);
                }
                componentMap.get(key).
                        put(AJAX_UPDATE, ajaxUpdate);
            }

            componentMap.get(key).
                    put(READONLY, true);
            componentMap.get(key).
                    put(MAX_VALUE, fieldStructure.getMaxValue());
            componentMap.get(key).
                    put(SHOULD_CHECK_NEGOTIF, fieldStructure.
                            getShouldCheckNegative());
    ...
    protected void createDimensionIksIgrek(FmsForm myForm) throws
        ...
        for (Map.Entry<String, MyField> entry : myForm.getFields().
                entrySet()) {
            ...
            if (myField.getNdAxis() != null) {
                switch (myField.getNdAxis()) {
                    case X_IKS:
                        dimensionIks.addAll(createDimensionFieldItems(myForm,
                                myField));
                        break;
                    case Y_IGREK:
                        dimensionIgrek.addAll(createDimensionFieldItems(myForm,
                                myField));
                        break;
                }
            }
     ...
     * @param myForm
     */
    protected void createDimensionZet(FmsForm myForm, boolean history) throws
            FormConfigException {
        ...
        for (MyField myField : myForm.getZetDimension()) {
            ...
            switch (myField.getItemsAsMyItems().getItemType()) {
                ...
                case doc:
                    ...
                    sb.append(" : ");
                    sb.append(loginController.getRolesAsSet());
                    String cacheKey = org.apache.commons.codec.digest.DigestUtils.
                            sha256Hex(sb.toString());

                    List<SelectItem> zetDimensionItems = appScopeSrvCtrl.
                            getCacheZetDimensionItems(cacheKey);
                    ...
                    if (zetDimensionItems == null) {
                        ...
                        if (history) {
                            cursor = filterService.
                                    createZetDimensionHistoryDocuments(myItems,
                                            myForm, getFilter());
                        } else {
                            cursor = filterService.
                                    createZetDimensionCurrentDocuments(myItems,
                                            myForm, getFilter());
                        }

                        Document modifiedView = new Document();

                        for (String viewKey : myField.getItemsAsMyItems().
                                getView()) {
                        ...
                        Document sortObject = myField.getItemsAsMyItems().
                                getSort();
                        String locale = myField.getItemsAsMyItems().
                                getLocale();

                        String labelStringFormat = myField.getItemsAsMyItems().
                                getLabelStringFormat();

                        zetDimensionItems = createSelectItems(cursor,
                                modifiedView, sortObject, labelStringFormat,
                                true, locale);

                        appScopeSrvCtrl.putCacheZetDimensionItems(cacheKey,
                                zetDimensionItems);
                    }
                    ...
                    if (myForm.getField(key).
                        ...
                        } else if (myForm.getField(key).
                                getMyconverter() instanceof NumberConverter) {
                            zetAxisConverter = new NumberConverter();
                        } else if (myForm.getField(key).
                                getMyconverter() instanceof MoneyConverter) {
    ...
    private List<SelectItem> createSelectItems(List<Document> cursor,
        ...
        if (sortObject == null || locale != null) {
            Collections.sort(lists, new Comparator() {
                ...
                public int compare(Object t1, Object t2) {
                    ...
                    return SELECT_PLEASE.equals(order1) ? 0 : collator.compare(
                            order1, order2);
                }
            });
        }
        ...
        if (all) {
            selectItems.add(
                    new SelectItem(SelectOneObjectIdConverter.NULL_VALUE,
                            SELECT_PLEASE));
        }
        ...
        for (Document object : lists) {
            ...
            Iterator<String> iterator = viewObject.keySet().
                    iterator();
            ...
            while (iterator.hasNext()) {
                ...
                if (fieldValue instanceof Date) {
                    toBeVieweeValues.add(SIMPLE_DATE_FORMAT__0.
                            format(fieldValue));
                } else {
