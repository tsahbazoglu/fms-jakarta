...
public class SimplePivotCtrlAdmin implements Serializable {
    ...
    public PivotRecord getRowData(String rowKey) {
        Map record = repositoryService.
                oneById(myForm.getDb(), myForm.getTable(), rowKey);
//TODO
    ...
    public void refreshLazy() {
        ...
        pivotRecords = new LazyDataModel<PivotRecord>() {
            ...
            @Override
            public List<PivotRecord> load(int first, int pageSize,
                    Map<String, SortMeta> sortBy,
                    Map<String, FilterMeta> filterBy) {
                return SimplePivotCtrlAdmin.this.load(null, false, first,
                        pageSize, myForm);
            }
        ...
        if (myForm != null && myForm.getKey() != null) {
            try {
                ...
                List<Map> cursor = repositoryService
                        .findSkipLimitAsLoggedUser(this.myForm.getDb(), myForm.
