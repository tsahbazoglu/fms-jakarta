...
public class PivotModifierCtrl extends PivotImpl {
    ...
    private String saveObject() throws Exception {
        ...
        for (CellMultiDimensionKey cellMultiKey : pivotData.keySet()) {
            ...
            CellMultiDimensionKey cellMultiDimensionKeyMap = eliminateMeasureCoordinate(
                    cellMultiKey);

            /**
             * we provide mapReduce emit phase by NULL checking
             */
            if (eliminatedToPureDimesnionCoordianteMap.get(
                    cellMultiDimensionKeyMap) == null) {
                eliminatedToPureDimesnionCoordianteMap.put(
                        cellMultiDimensionKeyMap, new Document());
            }
            ...
            eliminatedToPureDimesnionCoordianteMap.get(cellMultiDimensionKeyMap).
                    putAll(customOlapHashMap.toDocument());
        ...
        for (CellMultiDimensionKey cellMultiDimensionKey : eliminatedToPureDimesnionCoordianteMap.
                keySet()) {
            Document recordMapValue = eliminatedToPureDimesnionCoordianteMap.
            ...
            for (MyField myField : cellMultiDimensionKey.getCoordinates()) {
                ...
                if (indexRecord.get(fieldKey) == null && myField.
            ...
            if (recordMapValue.get(MONGO_ID, ObjectId.class) == null) {
                ...
                Bson searchPart = Filters.eq(MONGO_ID, recordMapValue.get(
                        MONGO_ID, ObjectId.class));
...
//                    }
    ...
    private CellMultiDimensionKey eliminateMeasureCoordinate(
            CellMultiDimensionKey cellMultiKey) {
        /**
         *
         ...
         * MAP PHASE
         */
        List<MyField> coordinates = new ArrayList<>(cellMultiKey.
                getCoordinates());
    ...
    private void refreshPivotData() throws NullNotExpectedException,
            MongoOrmFailedException,
            MoreThenOneInListException, UserException, FormConfigException {

        dataService.cleanMapReduce(getFilter());

        pivotData = dataService.createPivotData(formService.getMyForm(),
                getFilter(),
                getIksDimension(), getIgrekDimension(), getMapOfMeasureField());

        calcService.applyCalcCoordinates(pivotData, formService.getMyForm(),
                getFilter());
        ...
        if (formService.getMyForm().
            ...
            snapshotMapMultiDimension = dataService.createPivotDataSnapshot(
                    formService.getMyForm(), getFilter(),
                    getIksDimension(), getIgrekDimension(),
                    getMapOfMeasureField());
            calcService.provideSnapshotCalculatedValues(
    ...
    public String showEimza22() {
        ...
        if (list.isEmpty()) {
            ...
            dialogController.showPopupWarning(
                    ...
                    "2d_dialogControlAndSave");
        } else {
            List<Document> sortReferns = mongoDbUtil.find("iondb",
                    "setting_license_type_deneme");
            final Map<ObjectId, Integer> sortMap = new HashMap();
            for (Document nextElement : sortReferns) {
                sortMap.put((ObjectId) nextElement.get(MONGO_ID),
                        ((Number) nextElement.get("orderno")).intValue());
            }
    ...
    private void accesControlLevel2() {
        if (formService.getMyForm().
                getAccessControlLevelTwo() != null) {

            Document accessControlLevelTwo = formService.getMyForm().
                    getAccessControlLevelTwo();
            if (loginController.isUserInRole(accessControlLevelTwo.get(
                    ON_USER_ROLE))) {
                String dbName = (String) accessControlLevelTwo.get(FORM_DB);
                String piplineCode = accessControlLevelTwo.get("func",
                        String.class);

                if (piplineCode != null) {
                    piplineCode = piplineCode.replace(DIEZ, DOLAR);
                    Document commandResult = mongoDbUtil.runCommand(dbName,
                            piplineCode, getFilter());
...
//                                    message);
    ...
    public List findList(FmsForm myForm, Document searcheDBObject) throws
            Exception {
    ...
    public String downloadCsvFile() {
        downloadService
                .downloadCsvFilePivot(getIksDimension(), getIgrekDimension(),
                        pivotData, pivotViewerController.getPivotData());
        return null;
    }

    private void cleanData() {
        for (CellMultiDimensionKey cellMultiDimensionKey : pivotData.keySet()) {
            List<CustomOlapHashMap> customOlapHaspMaps = pivotData.get(
    ...
    public void submitRowData(AjaxBehaviorEvent event) {
        try {
            if ("001".equals(event.getComponent().
                    getAttributes().
                    get(VALUE))) {// FIXME move to engine-config pattern
                cleanData();
                setEditable(false);
                ...
                dialogController.showPopupError(
                        "\"Bildirime konu veri bulunuyor mu?\" alanı zorunlu alandır");
            }
        } catch (Exception ex) {
            logger.error("error occured", ex);
        }
    ...
    public String provideCrossCheck() {
        ...
        if (formService.getMyForm().
            ...
            if (rowObject.get(MONGO_ID) != null) {
                mongoDbUtil.updateMany(formService.getMyForm(), new Document(
                        MONGO_ID, rowObject.get(MONGO_ID)), new Document(
                        rowObject));
            } else {
                Document srchobj = new Document(getFilter());
                srchobj.put(FORMS, FIELDS_ROW);

                mongoDbUtil.updateMany(formService.getMyForm(), srchobj,
                        new Document(DOLAR_SET, new Document(rowObject)));
            }
        ...
        try {
            ...
            if (data == null || data.trim().
                ...
                //FIXME messagebundle
                dialogController.showPopupError(
                        "Kaydedilecek bir değişiklik tespit edilemedi");
                return null;
            }
            ...
            for (MyField coordinateRow : getIksDimension()) {
                ...
                for (MyField coordinateColumn : getIgrekDimension()) {
                    ...
                    if (customOlapHaspMaps != null && !customOlapHaspMaps.
                            isEmpty()) {
                        CustomOlapHashMap customOlapHaspMap = customOlapHaspMaps.
                                get(0);
                        ...
                        if (value == null || value instanceof String && value.
                            ...
                            if (value instanceof Number) {
                                ...
                                customOlapHaspMap.setValue((long) (Math.round(
                                        ((Number) value).doubleValue()) * 100));//veritabanına kuruş olarak gonderiyoruz
                            } else {
                                try {
                                    customOlapHaspMap.setValue((long) (Math.
                                            round(toModel.
                            ...
                            if ("kpbdb".equals(formService.getMyForm().
                                    getDb())) {
                                Number lastCheck = (Number) customOlapHaspMap.
                                        getValue();
                            ...
                            if (customOlapHaspMap.getUysformat() != null) {
                                div = ((NumberConverter) converter).getDiv(
                                        customOlapHaspMap.getUysformat());
                            }

                            if (value instanceof Number) {
                                value = Math.round(((Number) value).
                                        doubleValue() * div);//veritabanına kuruş olarak gonderiyoruz
            ...
            Map<String, List<MyConstraintFormula>> map = ctrlService.
                    constraintCrossCheckVersionTwo(getFilter(), pivotData);
        ...
        try {
            ...
            String successMessage = repositoryService
                    .sendForm(formService.getMyForm(),
                            (ObjectId) getSearchObjectValue(PERIOD), filter,
                            null);
