...
public class PivotDataModelReadonly implements PivotDataModel {
        ...
        if (pivotData != null
            ...
            if (customOlapHaspMaps != null && !customOlapHaspMaps.isEmpty()) {
                ...
                if (control != null) {
                    ...
                    style = String.format("%s background-color:%s;",
                            style == null ? "" : style, control.get(COLOR));
                    customOlapHaspMap.setStyle(style);
                    customOlapHaspMap.setControlID((ObjectId) control.get(
                            MONGO_ID));
                    customOlapHaspMap.setControlDesc(String.format(
                            "<hr/><b>kontrol</b> :  %s", control.
                                    get(DESCRIPTION)));
                }

                //provide snapshot diff values view for end user
                if (pivotDataSnapshot != null) {
                    List<CustomOlapHashMap> snapshot = pivotDataSnapshot.get(
                            cellMultiDimensionKey);
                    if (snapshot != null && !snapshot.isEmpty()) {
                        ...
                        if (snapshotValue instanceof Long) {
                            snapshotValue = ((Number) snapshotValue).
                                    doubleValue();
                        }

                        if (currentValue != null && snapshotValue != null && !currentValue.
                                equals(snapshotValue)) {

                            if (control == null) {
                                String style = (String) customOlapHaspMap.
                                        getStyle();
                                style = String.
                                        format("%s background:%s;", style,
                                                "rgb(126, 164, 221)");
                                customOlapHaspMap.setStyle(style);
                            }

                            customOlapHaspMap.
                                    setControlID(new ObjectId("dummy"));

                            StringBuilder controlDesc = new StringBuilder();
                            if (control != null) {
                                controlDesc.append(String.format(
                                        "<hr/><b>kontrol</b> :  %s", control.
                                                get(DESCRIPTION)));
                            }
                            ...
                            if (snapshotValue instanceof String) {
                                ...
                                if (snapshotValue.toString().
                                        trim().
                                        isEmpty()) {
                                    snapshotValue = "0";
                                }

                                myValue = Double.valueOf((String) snapshotValue).
                                        doubleValue();
                            ...
                            if (Double.isInfinite(myValue) || Double.isNaN(
                                ...
                                //FIXME messagebundle
                                controlDesc.append(String.format(
                                        "<hr/><b>önceki değer</b> :  %s", "BOŞ"));
                            } else {
                                controlDesc.append(String.format(
                                        "<hr/><b>önceki değer</b> :  %s",
                                        toView.format(Math.round(myValue))));
                            }

                            customOlapHaspMap.setControlDesc(controlDesc.
                                    toString());
    ...
    public void setNestedHeaders(List nestedHeaders) {
