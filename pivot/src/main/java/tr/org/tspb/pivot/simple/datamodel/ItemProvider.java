...
public class ItemProvider {
    ...
    private void init(MongoDbUtilIntr mongoDbUtil, String role, FmsForm myForm) {
        if (tedbirTuleriList == null) {
            ...
            Collections.sort(tedbirTuleriList,
                    new Comparator<ItemTedbirTuru>() {
                @Override
        ...
        if (kontrolTipleri == null) {
            ...
            //FIXME messagebundle
            List<Document> docs = mongoDbUtil.find("test",
                    "masak_anket_kontrol_tipi");
            for (Document doc : docs) {
        ...
        if (fields == null) {
            ...
            fields.add(new ItemField(myForm.getKey(), myForm.getPageName(),
                    FONT_STYLE));
