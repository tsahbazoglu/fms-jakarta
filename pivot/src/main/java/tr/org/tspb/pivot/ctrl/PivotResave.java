...
public class PivotResave extends PivotImpl {
    ...
    private String[] resaveForm = {"B2", "B3", "C1", "C2", "D1", "D2", "H1",
        "H2", "H3", "H4", "H5", "F11"};
    ...
    public List<SelectItem> getResaveAllTemplates() {
        if (resaveAllTemplates == null) {
            ...
            List<Document> cursor = mongoDbUtil.find(UYSDB, DATA_BANK_TEMPLATE,
                    new Document(FORMS, TEMPLATE), new Document(NAME, 1), null);

            for (Document object : cursor) {
                resaveAllTemplates.add(new SelectItem(object.get(MONGO_ID).
                        toString(), (String) object.get(NAME)));
            }
        }
    ...
    public List<SelectItem> getResaveMembers() {
        if (resaveMembers == null) {
            ...
            List<Document> cursor = mongoDbUtil.find(UYSDB, COMMON,
                    Filters.and(Filters.eq(FORMS, baseService.
                            getReportProperties().loginFormKey), Filters.eq(
                                    MONGO_LDAP_UID, new Document(DOLAR_NE,
                                            "empty"))),
            ...
            for (Document object : cursor) {
                resaveMembers.add(new SelectItem(object.get(MONGO_ID).
                        toString(), //
                        String.format("%s - %s", object.get(MONGO_LDAP_UID),
                                object.get(NAME))));
            }
        }
    ...
    public List<SelectItem> getResaveAllPeriods() {
        if (resaveAllPeriods == null) {
            ...
            List<Document> cursor = mongoDbUtil.find(UYSDB, COMMON,
                    new Document(FORMS, PERIOD), new Document(NAME, 1), null);

            for (Document object : cursor) {
                resaveAllPeriods.add(new SelectItem(
                        ((Number) object.get(VALUE)).intValue(),
    ...
    public String reSaveAll() throws Exception {
        ...
        List<Document> cursorMembers = mongoDbUtil.findWithProjection(UYSDB,
                COMMON,
                new Document(FORMS, MEMBER),
                new Document(MONGO_ID, true).append(NAME, true));
        for (Document member : cursorMembers) {
            cacheMapMember.put((ObjectId) member.get(MONGO_ID), (String) member.
                    get(NAME));
        }

        List<Document> cursorPeriods = mongoDbUtil.findWithProjection(UYSDB,
                COMMON,
                new Document(FORMS, PERIOD),
                new Document(MONGO_ID, true).append(NAME, true));
        for (Document period : cursorPeriods) {
            cacheMapPeriod.put((ObjectId) period.get(MONGO_ID), (String) period.
                    get(NAME));
        }
        ...
        for (String form : forms) {
            ...
            FmsForm myForm = ogmCreator
                    .getMyFormLarge(null, "graph", new Document(FORM, form),
                            getFilter(), loginController.getRoleMap(),
            ...
            Document periodQuery = new Document()
                    .append(VALUE, new Document().append(DOLAR_GTE,
                            resavePeriodStartStr).
                            append(DOLAR_LTE, resavePeriodEndStr));

            List<Document> cursorPeriod = mongoDbUtil.find(UYSDB, COMMON,
                    periodQuery);
            ...
            Document query = new Document()//
                    .append("workflowStatus.name", new Document(DOLAR_IN,
                            statusList))//
                    .
            ...
            List<Document> orgStatusList = mongoDbUtil.find(UYSDB,
            ...
            for (Document organizationStatus : orgStatusList) {
                 ...
                 */
                ObjectId memberID = (ObjectId) organizationStatus.get(
                        formService.getMyForm().
                                getLoginFkField());
                ObjectId periodID = (ObjectId) organizationStatus.get(PERIOD);
                ObjectId templateID = (ObjectId) organizationStatus.
                        get(TEMPLATE);

                Document member = mongoDbUtil.findOne(UYSDB, COMMON,
                        new Document(FORMS, formService.getMyForm().
                                getLoginFkField()).append(MONGO_ID, memberID));

                Document period = mongoDbUtil.findOne(UYSDB, COMMON,
                        new Document(FORMS, PERIOD).append(MONGO_ID, periodID));

                Document template = mongoDbUtil.findOne(UYSDB,
                        "dataBankTemplate",
                        new Document(FORMS, TEMPLATE).append(MONGO_ID,
                                templateID));
                ...
                pivotData = dataService.createPivotData(myForm, cellControl,
                        getIksDimension(), getIgrekDimension(),
                        getMapOfMeasureField());

                calcService.createPivotCalcData(pivotData, true, true, myForm,
                        getFilter());
                ...
                for (CellMultiDimensionKey cellMultiKey : pivotData.keySet()) {
                     ...
                     * MAP PHASE
                     */
                    List<MyField> listOfMappedCoordinates = new ArrayList<>(
                            cellMultiKey.getCoordinates());
                    ...
                    CellMultiDimensionKey cellMultiDimensionKeyMap = new CellMultiDimensionKey(
                            listOfMappedCoordinates);
                    ...
                    for (CustomOlapHashMap customOlapHaspMap : list) {
                        ...
                        if (mapReduce.get(cellMultiDimensionKeyMap) == null) {
                            mapReduce.put(cellMultiDimensionKeyMap,
                        ...
                        recordMap.append(measureField, customOlapHaspMap.
                                getValue());

                        if (recordMap.get(MONGO_ID) != null && !recordMap.get(
                                MONGO_ID).
                                equals(customOlapHaspMap.getId())) {
                         ...
                         * ON FLY REDUCE PHASE
                         */
                        List<Document> mappedList = mapReduce.get(
                                cellMultiDimensionKeyMap);

                        if (mappedList.isEmpty()) {
                            mappedList.add(recordMap);
                        } else {
                            Document prevRecordMap = mappedList.get(mappedList.
                                    size() - 1);
                ...
                // prepare and save
                for (CellMultiDimensionKey cellMultiDimensionKey : mapReduce.
                        keySet()) {
                    for (Document recordMapValue : mapReduce.get(
                         ...
                         */
                        for (MyField coordinate : cellMultiDimensionKey.
                                getCoordinates()) {
                            ...
                            indexRecord.put(coordinate.getKey(), coordinate.
                                    getId());
                            // FIXME we just hope that forms and field key have the same value
                        }

                        indexRecord.put(FORMS, myForm.getKey());
                        indexRecord.put(formService.getMyForm().
                                getLoginFkField(), memberID);
                        ...
                        ObjectId objectID = (ObjectId) recordMapValue.get(
                                MONGO_ID);
                        ...
                        if (objectID == null) {
                            ...
                            mongoDbUtil.updateMany(myForm, indexRecord,
                                    new Document(recordMapValue));
                        } else {
                            //recordMapValue.remove(_ID);
                            mongoDbUtil.updateOne(myForm.getDb(), myForm.
                                    getTable(), Filters.eq(MONGO_ID, objectID),
                                    recordMapValue);
                        }
                    }
                }
    ...
    protected String saveObject() throws Exception {
        ...
        calcService.createPivotCalcData(pivotData, true, true, formService.
                getMyForm(), getFilter());

        String collection = formService.getMyForm().
                getTable();
        ...
        for (MyField myField : formService.getMyForm().
            ...
            if (autosetKeyValue == null) {
                throw new UserException("Lütfen ".concat(myField.getName()).
                        concat(SECINIZ));
            }

            if (autosetKeyValue instanceof ObjectId) {
                if (SelectOneObjectIdConverter.NULL_VALUE.
                        equals(autosetKeyValue)) {
                    throw new UserException("Lütfen ".concat(myField.getName()).
                            concat(SECINIZ));
                }
        ...
        Object member = autosetMapValues.get(formService.getMyForm().
                getLoginFkField());
         ...
         */
        Document loginUserDBObject = (Document) mongoDbUtil.findOne(UYSDB,
                "common",
                new Document(MONGO_LDAP_UID, loginController.
                        getLoggedUserDetail().
                        getUsername()));
        ...
        for (CellMultiDimensionKey cellMultiKey : pivotData.keySet()) {
            ...
            for (CustomOlapHashMap customOlapHaspMap : list) {
                ...
                if (recordMap.get(MONGO_ID) != null && !recordMap.get(MONGO_ID).
                        equals(customOlapHaspMap.getId())) {
                    String errorMessage = "We do not expect more than one value in olap view at the save state. "
                            + "More than one record._id had been deteced on the cell." + cellMultiKey;
                    throw new Exception(errorMessage);
                ...
                if (mappedList.isEmpty()) {
                    ...
                    Document prevRecordMap = mappedList.get(
                            mappedList.size() - 1);
                    prevRecordMap.putAll(recordMap);
                }
            }
        ...
        for (CellMultiDimensionKey cellMultiDimensionKey : mapReduce.keySet()) {
            for (Document recordMapValue : mapReduce.get(cellMultiDimensionKey)) {
                ...
                for (MyField coordinate : cellMultiDimensionKey.getCoordinates()) {
                    ...
                    //FIXME we just hope that forms and field key have the same value
                    if (indexRecord.get(fieldKey) == null && coordinate.
                            getItemsAsMyItems() == null) {
                        indexRecord.put(fieldKey, coordinate.getDefaultValue());
                    }
                }

                indexRecord.put(FORMS, formService.getMyForm().
                        getKey());
                ...
                recordMapValue.put(OPERATOR_LDAP_UID, loginController.
                        getLoggedUserDetail().
                        getUsername());
...
//                        }
//                    }
                    mongoDbUtil.updateMany(formService.getMyForm(), searchPart,
                            updatePart);
                }
            }
        }
        ...
        repositoryService
                .runEventPostSave(new Document(getFilter()), formService.
                        getMyForm(), null);
