...
public class fpmx extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request,
        ...
        if (err) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST,
                    "error : fpmx : post method is not allowed with param : ".
                            concat(matchedParam));
            return;
        }
        ...
        if (!fphi.TEST && !VerifyRecaptcha.verify(gRecaptchaResponse)) {
            ...
            if (ppp.emailCount(email) == 1) {
                String token = FgtPswdTokenManager.instance().
                        getToken(email, 1);
                String link = request.getRequestURL().
                        ...
                        concat(token).
                        replace("fpmx", "fphi");
                String emailContent = MyWriteUtil.instance().
                        createEmailContent(
                                link);

                try {
                    ppp.sendMail("TSPB Parola Hatırlatma", emailContent, email,
