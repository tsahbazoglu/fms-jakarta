...
public class fpop extends HttpServlet {
    ...
    private static ServiceLoader<PPolicyProvider> serviceLoader = ServiceLoader.
            load(PPolicyProvider.class);
    ...
    private final Set<String> allowedPostAtrrs = new HashSet<>(Arrays.asList(
            PARAM_UPDATE_PSWD_TOKEN, PARAM_PSWD, PARAM_RE_PSWD, PARAM_RECAPTCHA));

    private Pattern strongRegex = Pattern.compile(
            "^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[!@#%\\*\\?\\+\\-])(?=.{8,})");
    private Pattern mediumRegex = Pattern.compile(
            "^(((?=.*[a-z])(?=.*[A-Z]))|((?=.*[a-z])(?=.*[0-9]))|((?=.*[A-Z])(?=.*[0-9])))(?=.{6,7})");
    ...
    protected void doPost(HttpServletRequest request,
        ...
        if (err) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST,
                    "ERROR : fpop : post method is not allowed with param : ".
                            concat(matchedParam));
        }
        ...
        if (Strings.isNullOrEmpty(updatePswdToken)) {
            response.sendError(HttpServletResponse.SC_BAD_REQUEST,
                    "error : fpop : suspicious action detected");
        } else if (!FgtPswdTokenManager.instance().
                isTokenValid(updatePswdToken, 0)) {
            ...
            String email = (String) FgtPswdTokenManager.instance().
                    getTokenData(updatePswdToken, 0);
            FgtPswdTokenManager.instance().
                    invalidateToken(updatePswdToken, 0);

            PPolicyProvider ppp = getPPolicyProivder();
            if (ppp == null) {
                response.sendError(HttpServletResponse.SC_NOT_IMPLEMENTED,
                        "PPolicy Provider not found");
                return;
            }
            ...
            try {
                ...
                request.getSession().
                        setAttribute("attrUpdateErrorMsg", ex.
                                getLocalizedMessage());
                response.sendRedirect(
                        request.getContextPath() + "/fphi/updateError");
                return;
            }
            ...
            String email = (String) FgtPswdTokenManager.instance().
                    getTokenData(updatePswdToken, 0);
            request.getSession().
                    setAttribute("shortToken", FgtPswdTokenManager.instance().
                            getToken(email, 0));
...
//    }
    private boolean validatePswdStrong(String pswd) {
        return strongRegex.matcher(pswd).
                find();
    }
