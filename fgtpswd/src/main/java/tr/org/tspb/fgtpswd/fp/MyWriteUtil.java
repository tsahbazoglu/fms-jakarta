...
public class MyWriteUtil {
    ...
    private static final String DEFAULT_MESSAGE_PATH = "tr.org.tspb.fp.config";
    ...
    private MyWriteUtil(String clientCode) {
        ...
        switch (clientCode) {
            case "TSPB":
                messages = ResourceBundle.getBundle(DEFAULT_MESSAGE_PATH,
                        trLocale);
                break;
            case "TDUB":
                messages = ResourceBundle.
                        getBundle("tr.org.tspb.fp.tdub.config", trLocale);
                break;
            case "KVHS":
                messages = ResourceBundle.
                        getBundle("tr.org.tspb.fp.kvhs.config", trLocale);
                break;
            default:
                messages = ResourceBundle.getBundle(DEFAULT_MESSAGE_PATH,
                        trLocale);
        ...
        InputStream styleInputStream = MyWriteUtil.class.getResourceAsStream(
                "style.css");
    ...
    public void writeWelcomePage(PrintWriter out, String desc, String err,
            String action, String homeaddr) {
        ...
        out.println(
                "              <input type='text' name='email' style='width:100%;text-align:center;'><br/><br/>");
        out.println("           </div>");
        out.println("           <div class='col-md-2'></div>");
        out.println("     </div>");
        ...
        out.println(
                "         <div class='g-recaptcha' data-sitekey='6LcQzG4UAAAAACk87rF4aMb07mwTiZnldOiqWh8s'></div>");
        out.println("       </div>");
        out.println("       <div class='col-md-6'>");
        out.println(
                "            <button style='background:#0074C8;color: #FFF;font-size: 15px;width: 100%;height: 75px;' type='submit'>");
    ...
    public void writeUpdatePswd(PrintWriter out, String updatePswdToken,
            boolean err, String action, String homeaddr) {
        ...
        sb.append(
                " : <span id='id-uys-update-pswd-counter' style='color:red;font-size:32px;'>");
        ...
        out.println("     <div class='form-group'>");
        out.println(
                "       <label class='col-md-4 control-label'>yeni parola</label>");
        out.println("       <div class='col-md-6'>");
        out.println(
                "         <input id='re-password-field' type='password' class='form-control' name='pswd' value=''>");
        out.println(
        ...
        out.println("     <div class='form-group'>");
        out.println(
                "       <label class='col-md-4 control-label'>yeni parola (tekrar)</label>");
        out.println("       <div class='col-md-6'>");
        out.println(
                "         <input id='password-field' type='password' class='form-control' name='re-pswd' value=''>");
        out.println(
        ...
        out.println("       <div class='col-md-6'>");
        out.println(
                "            <button id='id-update-pswd-submit-btn' style='background:#0074C8;color: #FFF;font-size: 15px;width: 100%;height: 75px;' type='submit'>");
        out.println("                <span>gönder</span>");
        out.println("            </button>");
        out.println("       </div>");
        out.println("     </div>");
        ...
        out.println(String.format("<input type='hidden' name='%s' value='%s'>",
                PARAM_UPDATE_PSWD_TOKEN, updatePswdToken));
    ...
    public void writeUpdatePswdFail(PrintWriter out, String url, String err,
            String homeaddr) {
        printHeadNoCaptcha(out);
        startBody(out);
        openContainer(out);
    ...
    public void writeBlocked(PrintWriter out, String homeaddr) {
        ...
        successMessage(out,
                "Sisteme giriş için TSPB : Üye İşleri Müdürlüğü ile irtibata geçiniz");
        closeContainer(out);
        out.println("</body>");
        out.println("</html>");
    }
    ...
    public void writeNotifyUser(PrintWriter out, String emailContent,
            String homeaddr) {
    ...
    public String createEmailContent(String link) {
        ...
        sb.append("<br/><br/>");
        sb.append(
                "Sistemimizde kayıtlı olan e-posta adresiniz üzerinden parola güncelleme işlemi talebinde bulunuldu.");
        sb.append("<br/><br/>");
        sb.append(
                "Eğer bu işlem sizin tarafınızdan yapılmadı ise bu e-posta içindeki talimatları dikkate almayınız.");
        sb.append("<br/><br/>");
        sb.
                append("Parolanızı güncellemek için aşağıdaki bağlantıyı kullanınız.");
        sb.append("<br/><br/>");
        sb.append(
                "Güncelleme bağlantısı tek seferlik olup geçerlilik süresi 1 saattir.");
        sb.append("<br/><br/>");
    ...
    private void scriptCounter(PrintWriter out) {
        ...
        out.println("     var n = " + String.valueOf(
                FgtPswdTokenManager.shortTermTokenTime + 1) + ";");
        out.println(
                "     e = document.getElementById('id-uys-update-pswd-counter');");
        out.println(
                "     btn = document.getElementById('id-update-pswd-submit-btn');");
    ...
    private void printTop(PrintWriter out, String homeAddr, String stepName) {
        ...
        out.println("        <img id='img_logo' src='" + messages.getString(
                "logo") + "'/>");
        out.println(
                "        <h5><a href=''><span class='title-style'>" + messages.
                        getString("website") + "</span></a></h5>");
        out.println("    </div>");
        out.println("    <div class='col-md-7'></div>");
        out.println("    <div class='col-md-4 text-right'>");
        out.println(
                "      <a href=" + (homeAddr == null ? "" : homeAddr) + "><h5>" + project_name + "</h5></a>");
        ...
        out.println("  <div class='col-md-12'>");
        out.println(
                "     <hr style='color: #0074C8;height: 2px;background-color: #0074C8;'>");
        out.println("  </div>");
        out.println("</div>");
    }
    ...
    private void homeLink(PrintWriter out, String url) {
        out.println("<div class='row'>");
        out.println(String.format(
                "<a class='col-md-12 text-center' href='%s'>%s</a>", url,
                "Üye Yönetim Sistemi"));
        out.println("</div>");
    }
