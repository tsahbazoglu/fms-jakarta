...
public abstract class AbstractViewer implements FmsOnFlyData, Serializable {
    ...
    public void refreshData1(final List list) throws NullNotExpectedException {
        ...
        data = new LazyDataModel<Map>() {
            ...
            @Override
            public List<Map> load(int first, int pageSize,
                    Map<String, SortMeta> sortBy,
                    Map<String, FilterMeta> filterBy) {
                List<Map> randomCars = new ArrayList<>();
                for (Object obj : list) {
                    try {
                        randomCars.add(mongoDbUtil.wrapIt(formService.
                                getMyForm(), (Document) obj));
    ...
    public Map retriveRowData(String rowKey) {
        ...
        Document object = mongoDbUtil.findOne(formService.getMyForm().
                getDb(), formService.getMyForm().
                        getTable(), new Document(MONGO_ID, id));
        return object;
    }
    ...
    public String getPickList() {
        return ComponentType.pickList.name();
    }

    public String getChips() {
        return ComponentType.chips.name();
    }
    ...
    public Object getSearchObjectValue(String key) {
        return filterService.getTableFilterCurrent().
                get(key);
    }
