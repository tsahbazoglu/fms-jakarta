...
public class AppScopeSrvCtrl {
    ...
    public MyProject getProject(String key, Document projectDoc) throws
            NullNotExpectedException, FormConfigException {
        MyProject myProject = cacheProjects.get(key);
        if (myProject == null) {
            myProject = ogmCreator.getMyProject(projectDoc, baseService.
                    getTagLogin());
    ...
    public Map<String, String> memberCacheNameById() {
        if (memberCacheNameById == null) {
            ...
            List<Document> cursor = mongoDbUtil.find(UYSDB, "common", Filters.
                    eq(FORMS, MEMBER));
    ...
    public AppScopeSrvCtrl() {
        ...
        try {
            ...
            Set<ObjectInstance> instances = platformMBeanServer.queryMBeans(
                    new ObjectName(MyJmxMonitor.objectName), null);

            if (instances != null && instances.iterator().
                    hasNext()) {
                ObjectName name = new ObjectName(MyJmxMonitor.objectName);
                platformMBeanServer.unregisterMBean(name);
            }

            instances = platformMBeanServer.queryMBeans(new ObjectName(
                    MyJmxMonitor.objectName), null);
            if (instances != null && !instances.iterator().
                    hasNext()) {
                ObjectName name = new ObjectName(MyJmxMonitor.objectName);
    ...
    public ObjectId getActivityStatusHayir() {
        ...
        if (activityStatusHayir == null) {
            activityStatusHayir = (ObjectId) mongoDbUtil
                    .findOne("iondb", FmsForm.ION_SETTING_ACTIVITY_STATUS,
                            new Document(CODE, ION_SETTING_ACTIVITY_STATUS_001)).
                    get(MONGO_ID);
        }
        return activityStatusHayir;
    }

    public MyFopFactory getMyFopFactory(String keyFileName) throws SAXException,
        ...
        MyFopFactory myFopFactory = new MyFopFactory(FopFactory.newInstance(
                new File(keyFileName)), factory);
    ...
    public Map<String, String> cacheIonSettingNotifyType() {
        if (cacheIonSettingNotifyType == null) {
            ...
            List<Document> cursor = mongoDbUtil.find("iondb",
                    "ion_setting_notify_type");
    ...
    public String getWebContext() {
        if (webContext == null) {
            String protocol = FacesContext.getCurrentInstance().
                    getExternalContext().
                    getRequestScheme();
            String serverName = FacesContext.getCurrentInstance().
                    getExternalContext().
                    getRequestServerName();
            String serverPort = ":".concat(String.valueOf(FacesContext.
                    getCurrentInstance().
                    getExternalContext().
            ...
            webContext = protocol.concat("://").
                    concat(serverName).
                    concat(
                            "https".equals(protocol) ? "" : serverPort).
                    concat(
                            contextPath);
        }
        return webContext;
    ...
    public void setTestControlFormulaTransferOrders(
            String testControlFormulaTransferOrders) {
    ...
    public void setDatabankRecalculateOnControl(
            Boolean databankRecalculateOnControl) {
    ...
    public Boolean getKunyeReport() {
        ...
        Document dbo = mongoDbUtil.findOne("configdb", "uyscrontab",
                new Document("job", "eimza_datafill_report"));
    ...
    private Document getPeriod(ObjectId periodId) {
        ...
        if (period == null) {
            period = mongoDbUtil.findOne(UYSDB, COMMON, new Document(MONGO_ID,
                    periodId));
    ...
    public Document cachePeriodByValue(Integer value) {
        ...
        if (period == null) {
            period = mongoDbUtil.findOne(UYSDB, COMMON, new Document(VALUE,
                    value));
    ...
    public Document cacheMemberByLdapUID(String ldapUID) {
        ...
        if (member == null) {
            member = mongoDbUtil.findOne(UYSDB, COMMON, new Document(
                    MONGO_LDAP_UID, ldapUID));
    ...
    private Document getTemplate(ObjectId templateId) {
        ...
        if (template == null) {
            template = mongoDbUtil.findOne(UYSDB, "dataBankTemplate",
                    new Document(MONGO_ID, templateId));
    ...
    public MyCalcDef getCalculateFormula(Map calculateFormulaSearchMap) {
        ...
        if (formula == null) {
            ...
            Document period = getPeriod((ObjectId) calculateFormulaSearchMap.
                    get(PERIOD));
            Document template = getTemplate(
                    (ObjectId) calculateFormulaSearchMap.get(TEMPLATE));
            ...
            Document findOneResult = mongoDbUtil.findOne(UYSDB,
                    "dataBankCalculateFormulas", query);
    ...
    public List<Document> getListOfPeriods(String db) {
        ...
        if (listOfPeriods == null) {
            ...
            List<Document> docs = mongoDbUtil.find(db, COMMON, new Document(
                    FORMS, PERIOD), new Document(NAME, 1), null);
    ...
    public ObjectId getPrevTemplate(String db, String sortKey, int prevCount,
        ...
        if (prevPeriodTemplateMap.get(searchMap) == null) {
            Document currentDBObject = mongoDbUtil.findOne(db,
                    "dataBankPreviousTemplate",
                    new Document().append(PERIOD, currentPeriodID).
                            append(
                                    TEMPLATE, currentTemplateId));

            if (currentDBObject == null) {
                throw new NullNotExpectedException(
                        "Önceki Dönem-Şablon İlişkisini Tespit Eden Fonksiyonda Hata Oluştu.");
            }

            int currentSummarizeOrder = ((Number) currentDBObject.get(
                    "summarize_order")).intValue();

            Document previousDBObject = mongoDbUtil.findOne(db,
                    "dataBankPreviousTemplate",
                    new Document("summarize_order", currentSummarizeOrder - 10));

            if (previousDBObject != null) {
                prevPeriodTemplateMap.put(searchMap,
                        (ObjectId) previousDBObject.get(TEMPLATE));
            } else {
    ...
    public Document getPrevPeriod(String db, String sortKey, int prevCount,
        ...
        String currentName = (String) mapId.get(currentId).
                get(sortKey);

        int currentIndex = listOfKeys.indexOf(currentName);
        if (currentIndex > 0) {
            String prevName = (String) listOfKeys.get(listOfKeys.indexOf(
                    currentName) - 1);
