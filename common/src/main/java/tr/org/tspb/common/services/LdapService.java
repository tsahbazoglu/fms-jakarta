...
public class LdapService extends AbstractSrv {
    ...
    public void updatePswdAndNotifyUser(String uid) throws LdapException,
            AddressException {
        try {
            new Updater(baseService.getProperties().
                    getLdapUsersDn())
                    .withUid(uid).
            ...
            throw new LdapException("could not update pswd and email on ldap",
                    ex);
        }
    }
    ...
    private MyLdapUser findUser(Map<String, Object> searchMap) throws
            LdapException {
        ...
        try {
            ...
            final NamingEnumeration answer = context.search(baseService.
                    getProperties().
                    getLdapUsersDn(), attr);
            if (answer != null) {
                try {
                    final SearchResult sr = (SearchResult) answer.next();
                    Attribute ldapUidAttr = sr.getAttributes().
                            get(UID);
                    Attribute emailAttr = sr.getAttributes().
                            get(LDAP_MAIL);

                    MyLdapUser myLdapUser = new MyLdapUser();
                    myLdapUser.setUid(ldapUidAttr.get().
                            toString());
                    if (emailAttr != null) {
                        myLdapUser.setEmail(emailAttr.get().
                                toString());
                    }
                    return myLdapUser;
                } catch (Exception ex) {
    ...
    public void removeUserToRole(String username, List<String> groupNames)
    ...
    public void addUserToRole(String username, List<String> groupNames) throws
            LdapException {
    ...
    private String createUserDN(String username) {
        return MessageFormat.format("uid={0}," + baseService.getProperties().
                getLdapUsersDn(), username);
    }

    private String createGroupDN(String groupname) {
        return MessageFormat.format("cn={0}," + baseService.getProperties().
                getLdapRolesDn(), groupname);
    }
    ...
    private String getAttribute(final String uid, final String attribute) {
        ...
        try {
            NamingEnumeration answer = ctxLocal.search(baseService.
                    getProperties().
                    getLdapUsersDn(), matchAttrs);
    ...
    private boolean isRoleExist(String role) {
        ...
        try {
            ...
            DirContext context = getContext();
            final NamingEnumeration searchResults = context.search(baseService.
                    getProperties().
                    getLdapRolesDn(), attr);
    ...
    private DirContext getContext() throws NamingException {
        ...
        if (ldapPswd == null || ldapUrl == null || ldapAdmin == null) {
            throw new RuntimeException(
                    "Ldap Central url and password is resolved to null");
        }
    ...
    public void sendMail(String uid, String email, String password) throws
            IOException, MessagingException, AddressException {
        String url = baseService.getWebContext();

        Document doc = mongoDbUtil.
                findOne(CONFIG_DB, COLLECTION_SYSTEM_MESSAGES,
                        new Document(CODE, "FORGET_PASSWORD"));

        if (doc != null && doc.get("contentQuery") != null) {

            Document fileQuery = (Document) doc.get("contentQuery");

            String dbName = (String) fileQuery.get(FORM_DB);
            String filename = (String) fileQuery.get("filename");

            List<GridFSFile> gridFSDBFiles = mongoDbUtil.findFiles(dbName,
                    filename);// gridFS.find(filename);
            ...
            InputStream inputStream = gridFSBucket.openDownloadStream(
            ...
            String content = MessageFormat.format(sw.toString(), url, uid,
                    password);

            byte[] attachment = mailService.createAttachment(doc, dbName);

            mailService.sendMail("TSPAKB - Üye Yönetim Sistemi hk.", content,
                    email, attachment);
