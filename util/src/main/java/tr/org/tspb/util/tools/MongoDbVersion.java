...
public class MongoDbVersion {
    ...
    private Long getOrGenerateGlobalRevisionNumber(String versionDB,
            String versionCollection, String recordType) {

        Document dbo = mongoDbUtil.findOne(versionDB,
                GLOBAL_LAST_REVISION_NUMBER_COLLECTION, new Document());

        long currentRevision = 1L;

        if (dbo == null) {
            mongoDbUtil.insertOne(versionDB,
                    GLOBAL_LAST_REVISION_NUMBER_COLLECTION, new Document(
            ...
            if (mongoDbUtil.findOne(versionDB, versionCollection, new Document(
                    REVISION, currentRevision).append(RECORD_TYPE, recordType)) != null) {

                currentRevision++;

                mongoDbUtil.updateMany(versionDB,
                        GLOBAL_LAST_REVISION_NUMBER_COLLECTION,
                        new BsonDocument(), new Document(LAST_REVISION,
                                currentRevision));
            }
        }
    ...
    public void archive( //
        ...
        history.put(REVISION, getOrGenerateGlobalRevisionNumber(versionDatabase,
                versionCollection, recordType));
    ...
    public Map<String, List> fetch(FmsForm myForm, String versionDB,
            String versionCollection, ObjectId recordID,
            List<String> importantFields) {

        List<DocumentRecursive> rowList = mongoDbUtil.findListAsName(versionDB,
                versionCollection,
                myForm, new Document(MASTER_RECORD.concat(DOT).
                        concat(_ID), recordID), null);
        ...
        for (String key : importantFields) {
            columnList.add(new Document()
                    .append("key", key).
                    append(TYPE, "string").
                    append(NAME, myForm.getField(key).
                            getName()).
                    append("datePattern", myForm.getField(key).
                            getDatePattern()).
