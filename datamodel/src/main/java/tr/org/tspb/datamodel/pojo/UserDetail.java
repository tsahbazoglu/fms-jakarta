...
public class UserDetail implements Serializable {
    ...
    public class EimzaPersonel {
        ...
        public EimzaPersonel(Document dbo, Document delegatedFieldRecord,
                String delegatedField, String delegatingField) {
            if (dbo.get("firstname") == null) {
                this.firstname = delegatedFieldRecord.get("adi").
                        toString();
            } else {
                this.firstname = dbo.get("firstname").
                        toString();
            }

            if (dbo.get("lastname") == null) {
                this.lastname = delegatedFieldRecord.get("soyadi").
                        toString();
            } else {
                this.lastname = dbo.get("lastname").
                        toString();
            }

            if (dbo.get("tcno") == null) {
                this.tcno = delegatedFieldRecord.get("kimlikNo").
                        toString();
            } else {
                this.tcno = dbo.get("tcno").
                        toString();
            }

            if (dbo.get("email") != null) {
                this.email = dbo.get("email").
                        toString();
            }
            if (dbo.get("telefon") != null) {
                this.telefon = dbo.get("telefon").
                        toString();
            }
            if (dbo.get("esign_provider") != null) {
                this.esign_provider = dbo.get("esign_provider").
                        toString();
            }
        ...
        public String toString() {
            ...
            sb.append("{");
            sb.append("'tcno':'".concat(tcno).
                    concat("', "));
            sb.append("'firstname':'".concat(firstname).
                    concat("', "));
            sb.append("'lastname':'".concat(lastname).
                    concat("'}"));
            return sb.toString();
        }
        ...
        private void validate() {
            if (this.tcno == null) {
                throw new RuntimeException(
                        "Could not detect logged or delegated user national id.");
            }
            if (this.firstname == null) {
                throw new RuntimeException(
                        "Could not detect logged or delegated user firstname.");
            }
            if (this.lastname == null) {
                throw new RuntimeException(
                        "Could not detect logged or delegated user lastname.");
            }
        }
