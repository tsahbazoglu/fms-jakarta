    ...
    public static class Build {
        ...
        public Build init() {
            ...
            for (String key : actions.keySet()) {
                ...
                if (roleMap.isUserInRole(viewerRole)) {
                    ...
                    map.put(key, Boolean.TRUE.equals(
                            ((Document) attrActionValue).get(ENABLE)));
                    String onUserRole = ((Document) attrActionValue).getString(
                            ON_USER_ROLE);
                    if (onUserRole == null) {
                        onUserRole = ((Document) attrActionValue).getString(
                                "onuserrole");
                    }
                    if (onUserRole == null) {
                        map.put(key, fmsScriptRunner.
                                runActionAsDbTableFilterResult(
                                        (Document) attrActionValue, roleMap,
                                        searchObject));
                    } else {
                    ...
                    for (Document document : listOfDoc) {
                        myActions.list.add(new DynamicButton(document,
                                this.myActions.fmsScriptRunner));
                    }
        ...
        public Build maskSaveWithCurrentCrudObject(MyMap crudObject) {
            ...
            if (!this.myActions.save) {
                this.myActions.save = this.myActions.create && crudObject.
                        isNew();
            }
        ...
        //
        private ActionEnableResult checkControlResultSchemaVersion110(
                Document filter, Document action) {
            ...
            if (func != null && !roleMap.isUserInRole(this.myActions.myForm.
                    getMyProject().
                    getAdminAndViewerRole())) {
                func = func.replace(DIEZ, DOLAR);

                Document commandResult = fmsScriptRunner
                        .runCommand(enable.getString(FORM_DB), func, filter,
                                roleMap.keySet());

                Object value = commandResult.get(RETVAL);
                controlResult.setEnable(Boolean.TRUE.equals(value));
            } else if (listOfChecks != null) {
                controlResult.setEnable(TagEventCheckListDoc.value(
                        fmsScriptRunner, filter, userDetail, roleMap,
                        listOfChecks));
            }
            ...
            if (controlResult.isEnable()) {
                ...
                if (gui instanceof Document) {
                    ...
                    controlResult.setDynamicButtonName(guiDoc.getString(
                            "caption"));

                    Document objectAction = action.get(ACTION, Document.class);

                    String actionFunc = objectAction.getString("func");
                    Document actionRef = objectAction.get(CONFIG_ATTR_REF,
                            Document.class);
                    ...
                    if (actionFunc != null) {
                        ...
                        if (actionRef.get("func") != null) {
                            controlResult.setMyaction(actionRef.get("func").
                                    toString());
                        }

                        if (actionRef.get(JAVA_FUNC) != null) {
                            controlResult.setJavaFunc(actionRef.getString(
                                    JAVA_FUNC));
                        }
                    }
                    controlResult.setSuccessMessage(((Document) gui).getString(
                            SUCCESS_MESSAGE));
                    controlResult.setFailMessage(((Document) gui).getString(
                            FAIL_MESSAGE));
                }
            }
