...
public class MyNotifies {
    ...
    public void reEnable(MyMap crud) {
        if (enableCode != null) {
            ...
            this.enable = scriptRunner.
                    runCommand("test", enableCode, tmp, roles).
                    getBoolean(RETVAL);
        }
    }
    ...
    public void reSubject(MyMap crud) {
        if (subjectCode != null) {
            Map tmp = new HashMap(crud);
            tmp.remove(ProjectConstants.INODE);
            this.subject = scriptRunner.runCommand("test", subjectCode, tmp,
                    roles).
                    getString(RETVAL);
        }
    }
    ...
    public void reContent(MyMap crud) {
        if (contentCode != null) {
            Map tmp = new HashMap(crud);
            tmp.remove(ProjectConstants.INODE);
            this.content = scriptRunner.runCommand("test", contentCode, tmp,
                    roles).
                    getString(RETVAL);
        }
    }
    ...
    public static class Builder {
        ...
        public Builder(List basicDBList, FmsScriptRunner scriptRunner,
                Set<String> roles) {
        ...
        private MyNotifies withDoc(Document doc, FmsScriptRunner scriptRunner,
                Set<String> roles) {
            ...
            if (enable instanceof Code) {
                ...
                x.enable = scriptRunner.runCommand("test", x.enableCode, null,
                        roles).
                        getBoolean(RETVAL);
            } else {
            ...
            if (to instanceof Code) {
                ...
                x.to = scriptRunner.runCommand("test", x.toCode, null, roles).
                        getString(RETVAL);
            } else {
            ...
            if (subject instanceof Code) {
                ...
                x.subject = scriptRunner.runCommand("test", x.subjectCode, null,
                        roles).
                        getString(RETVAL);
            } else {
            ...
            if (content instanceof Code) {
                ...
                x.content = scriptRunner.runCommand("test", x.contentCode, null,
                        roles).
                        getString(RETVAL);
            } else {
