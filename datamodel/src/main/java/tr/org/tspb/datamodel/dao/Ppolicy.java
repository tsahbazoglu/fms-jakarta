...
public class Ppolicy {
    ...
    public Map createTryCountSet() {
        return new Document(ProjectConstants.DOLAR_SET, new Document(_tryCount,
                tryCount));
    }
    ...
    public static class Builder {
        ...
        public Builder withMongoDBO(Map dbo) {
            ...
            this.ppolicy.changePswdTime = (Date) dbo.
                    get(Ppolicy._changePswdTime);
            this.ppolicy.lockTime = (Date) dbo.get(Ppolicy._lockTime);
            this.ppolicy.lock = Boolean.TRUE.equals(dbo.get(Ppolicy._lock));
            this.ppolicy.lockPeriodMinute = ((Number) dbo.get(
                    Ppolicy._lockPeriodMinute)).intValue();
            this.ppolicy.expirationDay = ((Number) dbo.get(
                    Ppolicy._expirationDay)).intValue();
            Object obj = dbo.get(Ppolicy._tryCount);
            if (obj instanceof Number) {
                this.ppolicy.tryCount = ((Number) obj).intValue();
            }
            this.ppolicy.maxTryCount = ((Number) dbo.get(Ppolicy._maxTryCount)).
                    intValue();
