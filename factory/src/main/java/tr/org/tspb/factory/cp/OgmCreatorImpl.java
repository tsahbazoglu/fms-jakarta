...
import tr.org.tspb.datamodel.dao.MyForm;
...
import static tr.org.tspb.constants.ProjectConstants.REPLACEABLE_KEY_WORD_FOR_THIS_FORM_KEY;
...
import tr.org.tspb.datamodel.dao.TagEventCheckListDoc;
...
public class OgmCreatorImpl implements OgmCreatorIntr {
    ...
    private boolean calcRendered(RoleMap roleMap, Document docField,
        ...
        List<Document> checkList = dboRendered.getList("check-list",
                Document.class);
        ...
        if (booleanValue != null) {
            ...
            if (onUserRole != null) {
                ...
                Document result = mongoDbUtil.findOne(db, collection,
                        (Document) query);
            ...
            return TagEventCheckListDoc.value(fmsScriptRunner, searchObject,
                    userDetail, roleMap, checkList);
        }
    ...
    private Object calcDefaultValue(MyProject myProject, Document docForm,
        ...
        List<String> listValue = defaultValue.
                getList("list-value", String.class);
        ...
        if (!FmsForm.SCHEMA_VERSION_111.equals(docForm.getString(
            ...
            if (funcValue != null) {
                ...
                try {
                    ...
                    if (docField.get(FORM_DB) == null) {
                        throw new Exception(
                                "db tag had not been set for field with defaultValue.");
                    }

                    Document commandResult = mongoDbUtil
                            .runCommand(docField.get(FORM_DB, String.class),
                                    funcValue, search, roleMap.keySet());

                    Object localDefaultValue = commandResult.get(RETVAL);

                    if (localDefaultValue instanceof Document) {
                        return ((Document) localDefaultValue).getObjectId(
                                MONGO_ID);
                    }
            ...
            List<Document> listOfRoleBasedValues = defaultValue.getList(
                    "list-of-role-based-values",
                    Document.class,
    ...
    private String resolveValue(Document docRoleValue, UserDetail userDetail,
        ...
        Document document = mongoDbUtil.findOne(db, table, filter);

        if (document != null) {
            return document.getString(projection);
        }
        return null;
    }
    ...
    public FmsForm getMyFormExternal(MyProject myProject, String collection,
        ...
        Document dboForm = mongoDbUtil.findOne("configdb", collection,
                new Document(formSearch));

        if (dboForm == null) {
            throw new NullNotExpectedException(
                    "no form regarding to formSearch query");
        }
        try {
            ...
            if (schemaVersion == null) {
                return new MyForm.Builder(myProject, dboForm, searchObject,
                        roleMap, userDetail, fmsScriptRunner, fmsRunMongoCmd)
                        .withOthers().
                        maskMultiUpload().
                        maskFilter().
                        maskAccesscontrol().
                ...
                switch (schemaVersion) {
                    case FmsForm.SCHEMA_VERSION_100:
                        return new MyForm.Builder(myProject, dboForm,
                                searchObject, roleMap, userDetail,
                                fmsScriptRunner, fmsRunMongoCmd)
                                .withOthers().
                                maskMultiUpload().
                                maskFilter().
                                maskAccesscontrol().
    ...
    public FmsForm getMyFormXsmall(MyProject myProject, Map searchObject,
        ...
        try {
            ...
            Document dboForm = cacheAndGetForm(myProject, myProject.
                    getConfigTable(), searchObject);
    ...
    public FmsForm getMyFormLarge(MyProject myProject, String collection,
        ...
        try {
            ...
            List<MyField> childFields = createChildFields(myProject, dboForm,
                    filter, roleMap, userDetail);
            ...
            if (schemaVersion == null) {
                return new MyForm.Builder(myProject, dboForm, filter, roleMap,
                        userDetail, fmsScriptRunner, fmsRunMongoCmd)
                        .withOthers().
                        maskMultiUpload().
                        maskFilter().
                        maskZetDimension().
                ...
                switch (schemaVersion) {
                    case FmsForm.SCHEMA_VERSION_100:
                        return new MyForm.Builder(myProject, dboForm, filter,
                                roleMap, userDetail, fmsScriptRunner,
                                fmsRunMongoCmd)
                                .withOthers().
                                maskMultiUpload().
                                maskFilter().
    ...
    @Override
    public MyProject getMyProject(Document docProject, TagLogin tagLogin) throws
     ...
     * @param projectKey
     * @return
     * @throws tr.org.tspb.constants.exceptions.NullNotExpectedException
     */
    @Override
    public MyProject getMyProject(String projectKey, TagLogin tagLogin) throws
            NullNotExpectedException, FormConfigException {
        Document project = mongoDbUtil.findOne("configdb", CFG_TABLE_PROJECT,
                new Document(FORM_KEY, projectKey));

        if (project == null) {
            throw new NullNotExpectedException(
                    "The related project has not been found");
        }
        ...
        if (FmsForm.SCHEMA_VERSION_110.equals(docForm.getString(
            ...
            for (Document docField : childFields) {
                ...
                MyField myField = new MyField.Builder(userDetail.getDbo().
                        ...
                        .maskAutoset((String) docForm.
                                get(FmsForm.SCHEMA_VERSION), roleMap).
                        maskShortName().
                        maskCode().
    ...
    private Map<String, MyField> createFields(MyProject myProject,
        ...
        if (docForm.get(FORMFIELDS) == null) {
            throw new NullNotExpectedException(
                    "fields property is resolved to null");
        }
        ...
        if (FmsForm.SCHEMA_VERSION_110.equals(docForm.getString(
            ...
            for (String fieldKey : keySet) {
                ...
                MyField myField = new MyField.Builder(userDetail.getDbo().
                        getObjectId(), myProject, docField, fmsScriptRunner)
                        .maskAutoset(docForm.getString(FmsForm.SCHEMA_VERSION),
                                roleMap).
                        maskShortName().
                        maskCode().
                        withRendered(calcRendered(roleMap, docField, filter,
    ...
    private Map<String, MyField> createRowFields(MyProject myProject,
        ...
        if (docForm.get(FIELDS_ROW) instanceof Document) {
            ...
            for (Map.Entry entry : mongoFieldsRow.entrySet()) {
                ...
                MyField myField = new MyField.Builder(userDetail.getDbo().
                        getObjectId(), myProject, docField, fmsScriptRunner)
                        .maskAutoset((String) docForm.get(MyForm.SCHEMA_VERSION),
                                roleMap).
                        maskShortName().
                        maskCode().
                        withRendered(calcRendered(roleMap, docField, filter,
    ...
    public MyField getMyField(FmsForm myForm, Document docField, Map filter,
        ...
        return new MyField.Builder(userDetail.getDbo().
                getObjectId(), myForm.getMyProject(), docField, fmsScriptRunner)
                ...
                maskAccesscontrol().
                maskNdTypeAndNdAxis().
                withDefaultValue(calcDefaultValue(myForm.getMyProject(), myForm.
                        getDbo(), docField, roleMap, filter, null)).
                maskComponentType().
                maskItemsAsMyItems(myForm.getSchemaVersion(), filter, roleMap.
                        isUserInRole(myForm.getMyProject().
                                getAdminRole()), roleMap.keySet()).
                withConverter(converter, null).
                maskRestOfThem().
                maskDescription().
                maskOrders().
    ...
    private class FmsScriptRunner1 implements FmsScriptRunner {
        ...
        public Document uysdbCommonFindOneLdapUID(String ldapUID) {
            return mongoDbUtil.findOne(UYSDB, COMMON, new Document(
