...
public class NumberConverter implements Converter, ConverterAttrs {
    ...
    public Object getAsObject(FacesContext context, UIComponent component,
        ...
        if (component.getAttributes().
                get(TREQUIRED) instanceof Boolean) {
            required = Boolean.TRUE.equals(component.getAttributes().
                    get(TREQUIRED));
        } else if (component.getAttributes().
                get(TREQUIRED) instanceof String) {
            required = "true".equals(component.getAttributes().
                    get(TREQUIRED));
        }
        ...
        FacesMessage facesMessageRequired = new FacesMessage(//
                ...
                MessageFormat.format("[{0}] {1}", label == null ? "" : label.
                        replaceAll("\\<.*?>", ""), MessageBundleLoader.
        ...
        FacesMessage facesMessageConverter = new FacesMessage(//
                ...
                MessageFormat.format("[{0}] pozitif {1} olmak zorundadır.", //
                        label == null ? "" : label.replaceAll("\\<.*?>", ""),
                        "integer".equals(instance) ? "tamsayı" : "sayı"),//
                "*");
        ...
        Object max = component.getAttributes().
                get("maxValue");//kuruş degil,TL anlamındadır
        ...
        boolean shouldCheckNegative = "true".equals(component.getAttributes().
                get("shouldCheckNegative"));

        if (component.getAttributes().
                get("shouldCheckNegative") instanceof Boolean) {
            shouldCheckNegative = Boolean.TRUE.equals(component.getAttributes().
                    get("shouldCheckNegative"));
        }
        ...
        try {
            ...
            Number minFractationDigits = (Number) component.getAttributes().
                    get(MIN_FRACTATION_DIGITIS);

            NumberFormat numberFormat = getNumberFormat(component,
                    minFractationDigits);

            //  Number number = numberFormat.parse(value);
            String converterFormat = (String) component.getAttributes().
                    get("converterFormat");
...
//                return null;
                ...
                facesMessageConverter = new FacesMessage(//
                        ...
                        MessageFormat.format(
                                "[{0}] {1} - {2} aralığında {3} olmak zorundadır.",//
                                label, 0D, maxAllowed, "integer".
                                        equals(instance) ? "tamsayı" : "sayı"),//
                        "*");
                throw new ConverterException(facesMessageConverter);
            } else {
        ...
        if (converterFormat != null) {
            switch (converterFormat) {
                ...
                case "##.##":
                    ...
                    break;
                default:
                    numberFormat = getNumberFormat(component,
                            minFractationDigits);
            }
        } else {
            numberFormat = getNumberFormat(component, minFractationDigits);
        }

        if (value instanceof Number) {
            if ("integer".equals(instance)) {
                value = ((Number) value).intValue() / div;
            } else {
                value = ((Number) value).doubleValue() / div;
            }
    ...
    public NumberFormat getNumberFormat(UIComponent component,
        ...
        if (minFractationDigits instanceof Number) {
            numberFormat.setMinimumFractionDigits(
                    ((Number) minFractationDigits).intValue());
        }
